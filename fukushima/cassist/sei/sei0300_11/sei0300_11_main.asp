...
' システム名: 教務事務システム
' 処  理  名: 個人別成績一覧
' ﾌﾟﾛｸﾞﾗﾑID : sei/sei0300_11/sei0300_11_top.asp
...
'           ■フレームページ
'-------------------------------------------------------------------------
' 作      成: 2006/01/27 西村　彩和子　福島高専用に新規作成
' 変      更: 2020/01/15 huy WEBアクセスログカスタマイズ
'*************************************************************************/
%>
<!--#include file="../../Common/com_All.asp"-->
<!--#include file="./sei0300_11_com.asp"-->
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Public  m_sTaisyo				'対象		'add 2020/01/15 huy
    Public  m_sSosa					'操作		'add 2020/01/15 huy
	Public  m_sUserId				'ログインID	'add 2020/01/15 huy
'///////////////////////////メイン処理/////////////////////////////
...
'********************************************************************************
    ...
    Do
		...
		'操作LOG出力
		Dim sTaisyo 
		sTaisyo = m_sTaisyo & "/"	& m_sGakusekiNo & "　"	& m_sName
		If gf_InsertOpeLog(m_iSyoriNen,"SEI0300","個人別成績一覧表",sTaisyo,m_sSosa,m_sUserId) <> 0 Then
			m_bErrFlg = True
			Exit Do
		End If
...
'response.write m_SchoolNo
    Do
		...
		w_sSql = w_sSql & vbCrLf & "  ,T16_GAKKA_CD "  
		w_sSql = w_sSql & vbCrLf & "  ,T16_HAITOGAKUNEN "  
		...
		w_sSql = w_sSql & vbCrLf & "  ,T16_HYOKA_FUKA_KBN AS FUKA_KBN "
...
'response.end
		...
		Do Until w_Rs.EOF
			...
			If w_bSelect = True Then
				Redim Preserve m_AryResult(8,i)
				...
				m_AryResult(4,i) = ""
				m_AryResult(5,i) = ""
				m_AryResult(6,i) = ""	'//修得済単位数
				m_AryResult(7,i) = ""	'//科目平均
				m_AryResult(8,i) = "0"	'//評価不可ﾌﾗｸﾞ
               ...
               m_AryResult(3,i) = w_Rs("JIKAN")			'//授業時間数
				...
				m_AryResult(8,i) = w_Rs("FUKA_KBN")		'//評価不可ﾌﾗｸﾞ
...
'				End If
				...
				IF f_GetTensuHyoka(m_AryResult(2,i)) <> "" Then
			    ELSE 
					IF m_AryResult(2,i) <> "" then
						IF cint(m_AryResult(2,i)) <> 0 then
							m_AryResult(6,i) = w_Rs("T16_HAITOTANI")	'//修得単位数
						END IF
					END IF
				end if
				...
				'科目平均取得
				'--------------------
				w_sSQL = "SELECT T35_HEIKIN FROM T35_KAMOKU_HEIKIN"
				w_sSQL = w_sSQL & " WHERE T35_NENDO =" & m_iSyoriNen
				w_sSQL = w_sSQL & " AND T35_SIKEN_KBN =" & cint(gf_SetNull2Zero(m_sSikenKBN))
'INS 2006/06/27 配当学年追加
				w_sSQL = w_sSQL & " AND T35_HAITOGAKUNEN ='" & w_Rs("T16_HAITOGAKUNEN") & "'"
				w_sSQL = w_sSQL & " AND T35_GAKKA_CD ='" & w_Rs("T16_GAKKA_CD") & "'"
				w_sSQL = w_sSQL & " AND T35_KAMOKU_CD ='" & w_Rs("T16_KAMOKU_CD") & "'"

		        iRet = gf_GetRecordset(w_RsKK, w_sSQL)
		        If iRet <> 0 Then
		            'ﾚｺｰﾄﾞｾｯﾄの取得失敗
					m_sErrMsg = "成績情報が取得できませんでした。"
		            f_GetResultData = 99
		            Exit Do
		        End If
				IF NOT w_RsKK.EOF Then
					m_AryResult(7,i) = w_RsKK("T35_HEIKIN")			'//科目平均
				END IF
				'//ﾚｺｰﾄﾞｾｯﾄCLOSE
				Call gf_closeObject(w_RsKK)
...
<!--#include file="../../Common/jsCommon.htm"-->
	...
	function window_onload() {
        ...
        document.frm.action="sei0300_11_topDisp.asp";
	...
	if (chk_Flg == false && p_FLG != 0) {f_NextPage(p_FLG);return false;} //変更がない場合はそのまま次へ

        if (!confirm("<%=C_TOUROKU_KAKUNIN%>")) {
           return ;
        }

        document.frm.action="sei0300_11_upd.asp";
        document.frm.target="main";
		document.frm.LOG_SOSA.value = "登録";		//add 2020/01/15 huy
		document.frm.LOG_TAISYO.value = document.frm.LOG_TAISYO.value + "/" + "<%=m_sGakusekiNo%>　<%=m_sName%>";		//add 2020/01/15 huy
		if( p_FLG == 1){
    ...
    function f_NextPage(p_FLG){
		...
		if( p_FLG == 1){
			document.frm.LOG_SOSA.value = "前へ";		//add 2020/01/15 huy
			...
			document.frm.LOG_SOSA.value = "次へ";		//add 2020/01/15 huy
		}

		document.frm.action="sei0300_11_main.asp";
		document.frm.target="_self";
		document.frm.LOG_TAISYO.value = document.frm.LOG_TAISYO.value;		//add 2020/01/15 huy
		document.frm.submit();
    }
	...
	<%Do %>
		...
		<tr>
			<th class="header" colspan="8">担　任　所　見</th>
		</tr>

		<tr>
	        <TD CLASS="detail" colspan="8">
				<textarea rows="4" cols="80" class="text" name="Syoken" onChange="chk_Flg=true;"><%=m_iSyoken%></textarea>
		...
		<!--明細部-->
		<tr>
			<th class="header" rowspan="2" width="25%">科　目　名</th>
			<th class="header" rowspan="2" width="12%">単位数</th>
			<th class="header" rowspan="2" width="12%">修得<br>単位数</th>
			<th class="header" rowspan="2" width="12%">成績<br>評価</th>
			<th class="header" rowspan="2" width="10%">平均</th>
			<th class="header" rowspan="2" width="12%">授業<br>時間数</th>
		...
		For i = 0 To m_iCnt
			...
			<tr>
				...
				<td class="<%=w_cell%>" align="right"><% IF m_AryResult(6,i)<> "" then %> <%=FormatNumber(cint(gf_SetNull2Zero(m_AryResult(6,i))),1)%> <% END IF %><br></td>
				<td class="<%=w_cell%>" align="right"><%=f_GetTensuHyoka(m_AryResult(2,i))%>　<%=m_AryResult(2,i)%></td>
				<td class="<%=w_cell%>" align="right"><%=m_AryResult(7,i)%><br></td>
			...
			w_iTaniKei_Syu= w_iTaniKei_Syu + cint(gf_SetNull2Zero(m_AryResult(6,i)))
		...
		<tr>
			...
			<td class="NOCHANGE" align="right"><%=FormatNumber(w_iTaniKei_Syu,1)%></td>
			<td class="NOCHANGE" align="right"><%=w_iHyokaKei%></td>
			<td class="NOCHANGE" align="right"></td>
		...
		<tr>
			...
			<td class="CELL2" colspan="4" align="right">席次　<%=m_iSikiji%>位／<%=m_iMemberCnt%>人中</td>
		</tr>
		</table>
