...
Sub s_SetParam()
	...
	m_iNendo	= session("NENDO")
	m_sKyokanCd	= session("KYOKAN_CD")
	'm_sGakuNo   = Replace(trim(Request("txtGakunen")),"@@@","")
...
%>
	...
	function f_ReLoadMyPage(){
		...
		document.frm.action="sei0300_11_top.asp";
	...
	<%End If%>
    	...
    	//試験区分
    	indx = document.frm.txtSikenKBN.selectedIndex;
    	if(indx >= 0 ){
    		txt = document.frm.txtSikenKBN.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//クラス
    	indx = document.frm.txtGakuNo.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtGakuNo.options[indx].text;
    		taisyo = taisyo + "/" + txt + "年";
    	}
    	
    	//年
    	indx = document.frm.txtClassNo.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtClassNo.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
	...
	<input type="hidden" name="txtGakunen" value="<%=m_sGakuNo%>">
