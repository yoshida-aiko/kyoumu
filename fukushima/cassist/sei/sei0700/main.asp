...
' 作      成: 2001/07/02 岩田
' 変      更: 2003/04/03 伊藤
'　　　　　　・当年度以前の成績一覧を表示するように修正（抽出条件から処理年度を削除）
...
'response.write " Where文 " & w_sWhere
    ...
    p_sSql = p_sSql & " T76_SIKENMEI||'/'||T76_GAKUNEN||'年/'||T76_CLASSMEI  LOG_TAISYO   , "			'add 2020/01/15 huy WEBアクセスログカスタマイズ
...
<link rel=stylesheet href=../../common/style.css type=text/css>
    ...
    //
    //************************************************************
	function f_detail(p_sUrl,p_taisyo){			//add 2020/01/15 huy
	//function f_detail(p_sUrl){		//del 2020/01/15 huy


		//add start 2020/01/15 huy
		//ログINSERT
		f_insertLog(p_taisyo,"詳細");
		//add end   2020/01/15 huy
	...
	//************************************************************
    //  [機能]  Downloadボタンクリック時の処理
    //  [引数]  なし
    //  [戻値]  なし
    //  [説明]
    //
    //************************************************************
	function f_download(p_sUrl,p_taisyo) {
		//ログINSERT
				...
				<tr>
					...
					<% else %>
						...
						<table border="1" width="610" class=hyo>
							<tr>
								<th align="center" height=16 class=header width="60"nowrap>年　度</th>
								<th align="center" height=16 class=header width="150"nowrap>試　験</th>
							...
							%>
								<tr>
									<td align="center" height="16" class=<%=w_cell%> width="60"nowrap><%=gf_HTMLTableSTR(m_Rs("T76_NENDO")) %>&nbsp</td>
									<td align="center" height="16" class=<%=w_cell%> width="150"nowrap><%=gf_HTMLTableSTR(m_Rs("T76_SIKENMEI")) %>&nbsp</td>
									...
									<!-- UPD START 2020/01/15 huy WEBアクセスログカスタマイズ -->
									<td align="center" height="16" class=<%=w_cell%> width="45"nowrap><input type=button class=button value="詳細" onclick="f_detail('../..<%=gf_HTMLTableSTR(m_Rs("T76_PATH")) %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>','<%=gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');"></td>
									<td align="center" height="16" class=<%=w_cell%> width="100"nowrap><a href='javascript:void(0);' onclick="f_download('../..<%=gf_HTMLTableSTR(m_Rs("T76_PATH")) %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>','<%=gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');">Download</a></td>
									...
									<!-- UPD END   2020/01/15 huy WEBアクセスログカスタマイズ -->
								</tr>
							<%
								w_iCnt = w_iCnt + 1
								m_Rs.MoveNext
