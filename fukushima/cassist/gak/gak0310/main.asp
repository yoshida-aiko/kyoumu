...
' 変      更: 2017/05/16 西村 学生写真データSession作成は global.aspで行う。
...
'2011.04.05 ins
    ...
    'Set Session("OraDatabasePh") = OraSession.GetDatabaseFromPool(100)
        ...
        if m_TxtMode = "Search" OR m_TxtMode = "LOG" then 
			'操作LOG出力
	        If gf_InsertOpeLog(m_iSyoriNen,"GAK0310","学生情報検索",m_sTaisyo,m_sSosa,m_sUserId) <> 0 Then
	            m_bErrFlg = True
	            Exit Do
	        End If
        End If
...
Sub s_MakeSQL(p_sSql)
    ...
    p_sSql = p_sSql & " ,A.T13_GAKUSEKI_NO||'/'|| A.T13_GAKUNEN||'年/'||C.M02_GAKKARYAKSYO"													'add 2020/01/15 huy WEBアクセスログカスタマイズ
    p_sSql = p_sSql & "||'/'||E.M05_CLASSMEI||'/'||B.T11_SIMEI||'/'||D.M01_SYOBUNRUIMEI||'/'||A.T13_GAKUSEI_NO  LOG_TAISYO    "			'add 2020/01/15 huy WEBアクセスログカスタマイズ
...
<link rel=stylesheet href=../../common/style.css type=text/css>
    ...
    //
    //************************************************************
    function f_detail(pGAKUSEI_NO,p_taisyo){		//add 2020/01/15 huy
//    function f_detail(pGAKUSEI_NO){		//del 2020/01/15 huy

			//add start 2020/01/15 huy
			//操作ログ出力
			document.frm.LOG_TAISYO.value = p_taisyo;
			document.frm.LOG_SOSA.value = "詳細";
	...
	'									m_Rs.Movefirst
										...
										Do Until m_Rs.EOF or w_iCnt >= m_iDsp
											...
											<tr>
												...
												<td align="center" height="16" class=<%=w_cell%>><input type=button class=button value="詳細" onclick="f_detail('<%= gf_HTMLTableSTR(m_Rs("T13_GAKUSEI_NO")) %>','<%= gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');"></td>
									...
									Do Until m_Rs.Eof or w_iCnt > m_iDsp
										...
										%>
											<td align="center" class=search width="150" valign="top">
												<!-- UPD START 2020/01/15 huy WEBアクセスログカスタマイズ -->
												<a href="javascript:f_detail('<%= gf_HTMLTableSTR(m_Rs("T13_GAKUSEI_NO")) %>','<%= gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');">
												...
												if w_bRet = True then
													%><IMG SRC="DispBinary.asp?gakuNo=<%= m_Rs("T13_GAKUSEI_NO") %>" width="88" height="136" border="0"><%
												Else
