...
Function f_GetDetailNyugaku()
	...
	Do
		...
		w_sSql = w_sSql &  vbCrLf &" 	A.T11_SYUSSINKO "
...
'*  [説明]  
'********************************************************************************
Function f_GetSyusinkoMei(p_sGakkoCd,p_iNyunendo)
	Dim w_iRet
	Dim w_sSQL
    ...
    w_sSQL = ""
    w_sSQL = w_sSQL & " SELECT "
    w_sSQL = w_sSQL & " 	 M31_NENDO	 "
    w_sSQL = w_sSQL & " 	,M31_GAKKO_CD "
    w_sSQL = w_sSQL & " 	,M31_GAKKOMEI "
    w_sSQL = w_sSQL & " FROM MM31_SYUSSINKO "
    w_sSQL = w_sSQL & " WHERE M31_GAKKO_CD = '" & p_sGakkoCd & "'"
    w_sSQL = w_sSQL & " 	AND M31_NENDO = " & p_iNyunendo
	...
	'// 出身校名
	f_GetSyusinkoMei = gf_SetNull2String(w_Rs("M31_GAKKOMEI"))
    '// 終了処理
    If Not IsNull(w_Rs) Then gf_closeObject(w_Rs)
...
'		m_BUKATUDOMEI 	  = m_Rs("M17_BUKATUDOMEI") 
		...
		m_NYUGAKU_CD      = m_Rs("T11_NYUGAKU_KBN")
		m_SYUSSINKO 	  = f_GetSyusinkoMei(gf_SetNull2String(m_Rs("T11_SYUSSINKO")),Session("HyoujiNendo"))
	End if
...
-->
					...
					<% '編入学、転入学の場合のみ表示
					   If cstr(m_NYUGAKU_CD) = cstr(C_NYU_HENNYU) Or cstr(m_NYUGAKU_CD) = cstr(C_NYU_TENNYU) Then %>
						<% 'if gf_empItem(C_T11_SYUSSINKO) then %>
							<tr>
								<td class="disph" height="16">出身校</td>
					...
					<% if gf_empItem(C_T11_NYU_SEISEKI) then %>
						<tr>
							<td class="disph" height="16">入 試 成 績</td>
							<td class="disp"><%= m_NYU_SEISEKI %>&nbsp</td>
						</tr>
					<% End if %>
				</table>
