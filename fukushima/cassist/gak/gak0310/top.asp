...
' 変      更: 2002/06/05 shin
' 変      更: 2015/03/20 kiyomoto Win7対応
...
Sub s_MakeGakkaWhere()
    ...
    s_sGakkaWhere = ""
    s_sGakkaWhere = s_sGakkaWhere & " M02_NENDO = " & m_iHyoujiNendo & " AND "
    s_sGakkaWhere = s_sGakkaWhere & " M02_GAKKA_CD <> '00'"

'response.write s_sGakkaWhere
...
<link rel=stylesheet href=../../common/style.css type=text/css>
    ...
    function f_Search(){
    	...
    	//学年
    	indx = document.frm.txtGakunen.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtGakunen.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	    	
    	
    	//学科
    	indx = document.frm.txtGakka.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtGakka.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//クラス
    	indx = document.frm.txtClass.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtClass.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//性別
    	indx = document.frm.txtSeibetu.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtSeibetu.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//名称
    	len = document.frm.txtName.value.length;
    	if(len > 0){
    		txt = document.frm.txtName.value;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//学籍番号
    	len = document.frm.txtGakusekiNo.value.length;
    	if(len > 0){
    		txt = document.frm.txtGakusekiNo.value;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//学生番号
    	len = document.frm.txtGakuseiNo.value.length;
    	if(len > 0){
    		txt = document.frm.txtGakuseiNo.value;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//出身中学校
    	len = document.frm.txtTyugaku.value.length;
    	if(len > 0){
    		txt = document.frm.txtTyugaku.value;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//中学校クラブ
    	indx = document.frm.txtTyuClub.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtTyuClub.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//現在クラブ
    	indx = document.frm.txtClub.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtClub.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//寮
    	indx = document.frm.txtRyoseiKbn.selectedIndex;
    	if(indx > 0 ){
    		txt = document.frm.txtRyoseiKbn.options[indx].text;
    		taisyo = taisyo + "/" + txt;
    	}
    	
    	//画像検索する
    	if(document.frm.CheckImage.checked == true){
    		taisyo = taisyo + "/画像検索する";
    	}
    
    	//最初のスラッシュ削除
    	if(taisyo.length > 0){
    		taisyo = taisyo.substr(1);
    	}
		...
		document.frm.LOG_TAISYO.value = taisyo;		//add 2020/01/15 huy
		document.frm.LOG_SOSA.value = "表示";		//add 2020/01/15 huy
        document.frm.submit();
...
//-->
							...
							<td>
								<%IF m_sGakunen <> "@@@" and m_sGakunen <> "" then 
									...
									<!-- 2015.03.20 Upd width:110->180 -->
									<select name="txtClass" DISABLED style="width:180px;">
									<option value="@@@">　　　　　　　</option>
									</select>
							    <% end if %>
							</td>
