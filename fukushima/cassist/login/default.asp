...
' 作      成: 2001/07/02 
' 変      更: 2001/07/26    モチナガ
' 変      更: 2016/09/09　高口　認められたIPのみログインするように変更
'             2017/03/03  藤 美由紀 IPチェックはユーザチェックの後に変更
'             2018/09/26  清本 管理マスタのIPチェック時に年度条件を追加
'             2020/01/15  huy IPチェック処理を外す
'*************************************************************************/
%>
...
Sub Main()
    ...
    Dim w_sErrLoginMsg      '// ﾛｸﾞｲﾝ用ｴﾗｰﾒｯｾｰｼ
    ...
    w_sErrLoginMsg = ""
     ...
     Do
		...
		w_IpAddress = Request.ServerVariables("REMOTE_ADDR") 'Ins 2016/09/08 Kouguchi
			...
			'// ﾛｸﾞｲﾝﾁｪｯｸ
			If Not f_login(w_LoginID,w_PassWord,w_sErrLoginMsg) Then
				Call ErrPage(w_sErrLoginMsg)          '// エラーページを表示
				Exit Do						'add 2017/03/03 m.tou
			End if

			'2017/03/03 m.tou IPチェックをログインチェックの後に移動
			'// IPｱﾄﾞﾚｽﾁｪｯｸ
			'If Not f_loginIp(w_LoginID,w_IpAddress,w_sErrLoginMsg) Then      'コメントアウト 2020/01/15 huy
				'Call ErrPage(w_sErrLoginMsg)          '// エラーページを表示      'コメントアウト 2020/01/15 huy
				'Exit Do                                                      'コメントアウト 2020/01/15 huy
			'End if                                                           'コメントアウト 2020/01/15 huy
...
'*          p_IpAddress = 端末のIPアドレスﾞ
'*  [戻値]  なし
'*  [説明]  
'********************************************************************************
Function f_loginIp(p_id,p_IpAddress,p_ErrLoginMsg)
    Dim w_sSql
    ...
    f_loginIp = false
    p_ErrLoginMsg = "IPアドレスエラー\nこのパソコンのIPアドレスは接続が認められていません。\n認められたIPアドレスが設定されたパソコンから接続してください。"

    '// Nullなら抜ける
    if trim(p_id) = "" then
        Exit Function
    Elseif trim(p_IpAddress) = "" then
        Exit Function
    End if
    ...
    w_sSql = w_sSql & "   ('" & p_IpAddress & "' IN ( M10_IP_1, M10_IP_2, M10_IP_3)  OR"
'    w_sSql = w_sSql & "    '" & p_IpAddress & "' IN (SELECT M00_KANRI FROM MM00_KANRI WHERE M00_NO BETWEEN 55 AND 59) )"
    w_sSql = w_sSql & "    '" & p_IpAddress & "' IN (SELECT M00_KANRI FROM MM00_KANRI WHERE M00_NO BETWEEN 55 AND 59 AND M00_NENDO = " & session("NENDO") & " ) )"	'2018.09.26 Upd Kiyomoto
...
'*  [説明]  
'********************************************************************************
Function f_login(p_id,p_pass,p_ErrLoginMsg)
    Dim w_sSql
...
'********************************************************************************
'*  [機能]  ログインエラー表示ページ
'*  [引数]  p_sErrLoginMsg 表示するエラーメッセージ
'*  [戻値]  なし
'*  [説明]  
'********************************************************************************
Sub ErrPage(p_sErrLoginMsg)
%>
 <HTML>
   <HEAD>
