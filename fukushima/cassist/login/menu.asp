...
' 変      更: 2001/07/26    モチナガ
'           : 2003/02/14    ヤノ
'           : 2006/04/27    西村 福島高専用　学生顔写真検索情報追加
'*************************************************************************/
%>
...
Const mC_SCHOOL_NO_FUKUSHIMA = 11 
...
<!--2003/07/07 del takada <tr><td><a class=menu href="../sei/sei0800/" target="<%=C_MAIN_FRAME%>">成績参照</a></td></tr> -->
	...
	<% End If%>
		...
		<% if Cint(m_SchoolNo) = Cint(mC_SCHOOL_NO_NUMAZU) then %>
			<% if gf_empMenu("SAISI0500") then %><tr><td><a class=menu href="../saisi/saisi0500/" target="<%=C_MAIN_FRAME%>">不合格学生一覧(担)</a></td></tr><% End if %>
			<% if gf_empMenu("SAISI0501") then %><tr><td><a class=menu href="../saisi/saisi0501/" target="<%=C_MAIN_FRAME%>">不合格学生一覧</a></td></tr><% End if %>
		<% end if %>
		<!--2003/10/27 -->

		<% if Cint(m_SchoolNo) = Cint(mC_SCHOOL_NO_FUKUSHIMA) then %>
			<% if gf_empMenu("SEI0300") then %><tr><td><a class=menu href="../sei/sei0300_11/" target="<%=C_MAIN_FRAME%>">個人別成績一覧</a></td></tr><% End if %>
		<% else %>
...
　　　	<% if gf_empMenu("GAK0300") then %><tr><td><a class=menu href="../gak/gak0310/" target="<%=C_MAIN_FRAME%>">学生情報検索</a></td></tr><% End if %>
	...
	<% Case "SUPPORT" %>
		...
		<!--2006/04/27 西村 福島高専用 ins -->
		<% if Cint(m_SchoolNo) = Cint(mC_SCHOOL_NO_FUKUSHIMA) then %>
			<% if gf_empMenu("GAK0350_11") then %><tr><td><a class=menu href="../gak/gak0350_11/" target="<%=C_MAIN_FRAME%>">学生顔写真情報検索</a></td></tr><% End if %>
		<% end if %>
		<!--2003/10/27 -->
