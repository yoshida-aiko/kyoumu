...
'2001/12/14 Add ---->
            w_sSQL = w_sSQL & vbCrLf & "AND T20.T20_GAKKI_KBN=" & Session("GAKKI") & " "
'2001/12/14 Add <----
...
'response.end
        ...
        If w_iRet <> 0 Then
            ...
            m_sErrMsg = "レコードセットの取得に失敗しました" & chr(13)
'            m_sErrMsg = m_sErrMsg & w_sSQL
...
dim w_iCourseCD
    ...
    Do Until m_Rs.EOF
        if CInt(m_Rs("T20_YOUBI_CD")) = CInt(m_iYobiCnt) and CDbl(m_Rs("T20_JIGEN")) = CDbl(m_iJgnCnt) Then
			if isnull(m_Rs("T20_DOJI_JISSI_FLG")) = true then
...
Function f_GetSentaku()
    ...
    Do
		...
		w_sSQL = w_sSQL & vbCrLf & "AND T18.T18_GAKKA_CD = '" & f_GetClassGakka(m_iSyoriNen,m_iGakunen,m_iClass) & "'"
...
'*          p_iGakuNen：学年
'*          p_ClassNo ：クラスNO
'*  [戻値]  f_GetClassGakka：クラス名
'*  [説明]  
'********************************************************************************
Function f_GetClassGakka(p_iNendo,p_iGakuNen,p_ClassNo)
	Dim w_iRet
	Dim w_sSQL
	Dim rs
    ...
    Do Until m_Rs.EOF
        If CInt(m_Rs("T20_YOUBI_CD")) = CInt(m_iYobiCnt) And CDbl(m_Rs("T20_JIGEN")) = CDbl(m_iJgnCnt) Then
			...
			if isnull(m_Rs("T20_DOJI_JISSI_FLG")) = true then
		        ...
		        w_sSQL = ""
		        w_sSQL = w_sSQL & "SELECT "
		        w_sSQL = w_sSQL & "T27_KYOKAN_RENMEI "
		        w_sSQL = w_sSQL & "From "
		        w_sSQL = w_sSQL & "T27_TANTO_KYOKAN "
		        w_sSQL = w_sSQL & "Where "
		        w_sSQL = w_sSQL & "T27_NENDO = " & m_iSyoriNen & " "
		        w_sSQL = w_sSQL & "And "
		        w_sSQL = w_sSQL & "T27_GAKUNEN = " & m_iGakunen & " "
		        w_sSQL = w_sSQL & "And "
		        w_sSQL = w_sSQL & "T27_CLASS = " & m_iClass & " "
		        w_sSQL = w_sSQL & "And "
		        w_sSQL = w_sSQL & "T27_KAMOKU_CD = '" & m_Rs("T20_KAMOKU") & "' "

		        Set w_Rs = Server.CreateObject("ADODB.Recordset")
		        w_iRet = gf_GetRecordsetExt(w_Rs, w_sSQL, w_iDsp)
		response.write("<!-- " & w_sSQL & "-->")
		        ...
		        m_sKyokan = w_Rs("T27_KYOKAN_RENMEI")
		        w_Rs.Close
		        Set w_Rs = Nothing

		        Exit Do
			else
		response.write("<!-- " & m_Rs("T20_HYOJI_KYOKAN") & "-->")
		        m_sKyokan = m_Rs("T20_HYOJI_KYOKAN")
		        Exit Do
