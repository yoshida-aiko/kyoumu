...
'操作ログを登録					gf_InsertOpeLog(p_nendo,p_syoriId,p_syoriName,p_taisyo,p_sosa,p_userId)		'add 2020/01/15 huy
		...
		if cint(w_iSyubetu) = C_K_KEKKA_RUISEKI_SIKEN then
			...
			for w_num = cint(p_sSikenKbn)-1 to 1 Step -1
				...
				if gf_SetNull2String(w_UpdateDate) <> "" then
					...
					if not gf_GetShikenDate(w_iNendo,p_sGakunen,w_num+1,p_sKaisibi,"START") then 
						exit function
					end if
