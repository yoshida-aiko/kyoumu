...
'	End If
	...
	if cstr(m_sSyozokuGakka) = "00" then
		m_sGakkaWhere = m_sGakkaWhere & " AND M02_GAKKA_CD <> '" & m_sSyozokuGakka & "' "
	else
		m_sGakkaWhere = m_sGakkaWhere & " AND M02_GAKKA_CD = '" & m_sSyozokuGakka & "' "
	end if
...
'	End If
	...
	''新規登録時
	If m_sMode = "Touroku" Then
		m_sKamokuWhere = " T15_NYUNENDO = " & cint(w_sNendo)  & ""	'//入学年度=処理年度-学年+1(初期表示時は1年生を表示の為,学年ｰ1を省略)
		m_sKamokuWhere = m_sKamokuWhere & " Group By T15_NYUNENDO , T15_KAMOKU_CD , T15_KAMOKUMEI "
	...
	''学科未選択時は、科目は未選択
	if m_sGakkaCD = "@@@" Or m_sGakkaCD = "" then
		m_sKamokuWhere = " T15_NYUNENDO = " & (cint(w_sNendo) - cint(m_sGakunenCD) + 1)  & ""	'//入学年度=処理年度-学年+1
		m_sKamokuWhere = m_sKamokuWhere & " Group By T15_NYUNENDO , T15_KAMOKU_CD , T15_KAMOKUMEI "
		m_sKamokuOption = " DISABLED "
		m_sKamokuCD = "@@@"
		Exit Sub
	...
	''全学科
	If (cstr(m_sGakkaCD) = cstr(C_CLASS_ALL)) or (cstr(m_sGakkaCD) = cstr("00")) Then
		m_sKamokuWhere = " T15_NYUNENDO = " & (cint(m_sNendo) - cint(m_sGakunenCD) + 1) & ""	'//入学年度=処理年度-学年+1
		m_sKamokuWhere = m_sKamokuWhere & " AND T15_KAMOKU_KBN = " & C_KAMOKU_IPPAN

		If m_sCourseCD <> "@@@" AND m_sCourseCD <> "" Then
			m_sKamokuWhere = m_sKamokuWhere & " AND T15_COURSE_CD = " & m_sCourseCD &""
		End IF
		m_sKamokuWhere = m_sKamokuWhere & " AND T15_KAISETU" & m_sGakunenCD & "="  & m_sGakkiCD
		m_sKamokuWhere = m_sKamokuWhere & " Group By T15_NYUNENDO , T15_KAMOKU_CD , T15_KAMOKUMEI "
	Else
		m_sKamokuWhere= " T15_NYUNENDO = " & (cint(m_sNendo)  - cint(m_sGakunenCD) + 1) & ""
		m_sKamokuWhere = m_sKamokuWhere & " AND T15_GAKKA_CD = " & m_sGakkaCD &""
		m_sKamokuWhere = m_sKamokuWhere & " AND T15_KAISETU" & m_sGakunenCD & "=" & m_sGakkiCD
		m_sKamokuWhere = m_sKamokuWhere & " AND T15_KAMOKU_KBN <> " & C_KAMOKU_IPPAN

		If cstr(gf_SetNull2String(m_sCourseCD)) <> "@@@" AND tirm(cstr(gf_SetNull2String(m_sCourseCD))) <> "" Then
			m_sKamokuWhere = m_sKamokuWhere & " AND T15_COURSE_CD = " & m_sCourseCD &""
			m_sKamokuWhere = m_sKamokuWhere & " Group By T15_NYUNENDO , T15_KAMOKU_CD , T15_KAMOKUMEI "
		End If
	End IF

'response.write m_sKamokuWhere & "<BR>"
...
<title>使用教科書登録</title>
	...
	function f_CheckData() {
		...
		if( f_Trim(document.frm.txtKamokuCD.value) == "@@@" ){
			...
			if(!document.frm.txtKamokuCD.disabled){
		...
		<td align="left">
			<table width="100%" border=1 CLASS="hyo">
				...
				<%	'共通関数から科目に関するコンボボックスを出力する
					...
					call gf_ComboSet("txtKamokuCD",C_CBO_T15_RISYU,m_sKamokuWhere,"style='width:175px;'" & m_sKamokuOption,True,m_sKamokuCD)%>
				<span class=hissu>*</span>
				</td>
				</tr>
				<tr>
