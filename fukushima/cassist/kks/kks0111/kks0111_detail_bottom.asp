...
Sub s_SetParam()
	...
	m_sFromDate = gf_YYYY_MM_DD(request("FromDate"),"/")
	m_sToDate = gf_YYYY_MM_DD(request("ToDate"),"/")
...
function f_SetKekka()
	...
	w_sSQL = w_sSQL & " 	T21_KAMOKU, "
	w_sSQL = w_sSQL & " 	T21_KAMOKU_KBN "
	w_sSQL = w_sSQL & " from "
	w_sSQL = w_sSQL & " 	T21_SYUKKETU, "
	w_sSQL = w_sSQL & " 	M04_KYOKAN "
	
	w_sSQL = w_sSQL & " where "
	w_sSQL = w_sSQL & "  	 T21_KYOKAN = M04_KYOKAN_CD(+) "
	w_sSQL = w_sSQL & "  and T21_NENDO = M04_NENDO "
...
function f_Get_SyukketuKbn()
	...
	If gf_GetRecordset(w_Rs_Kekka,w_sSQL) <> 0 Then exit function
	        			...
	        			<% if not (m_Rs is nothing) then %>
					    	<% Do until m_Rs.EOF %>
								...
								<tr>
									...
									<td width="150" class="<%=w_Class%>" align="center" nowrap><%=gf_GetKamokuMei(m_iSyoriNen,m_Rs("T21_KAMOKU"),m_Rs("T21_KAMOKU_KBN"))%></td>
