...
' システム名: 教務事務システム
' 処  理  名: 使用教科書登録
' ﾌﾟﾛｸﾞﾗﾑID : web/web0320/web0320_main.asp
' 機	  能: 使用教科書の登録を行う
'-------------------------------------------------------------------------
...
' 変	  更: 2002/05/02 進   浩人 所属学科のみ削除可能に修正
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	public m_Disabled		'2002/5/2 shin button disabled
...
Sub s_SetParam()
	...
	m_iNendo	 = request("hidYear")					   '年度
	m_iGakunen = trim(replace(Request("hidGakunen"),"@@@",""))
	m_sGakkaCd = trim(replace(Request("hidGakka"),"@@@",""))
	...
	If Request("txtMode") = "" Then
		m_sPageCD = 1
	Else
		m_sPageCD = cint(gf_SetNull2Zero(request("txtPageCD")))	':表示済表示頁数（自分自身から受け取る引数）
	End If
...
function f_GetData()
	...
	w_sSQL = w_sSQL & vbCrLf & "  and T15_NYUNENDO (+)= (" & m_iNendo & "-T47_GAKUNEN+1) "
	w_sSQL = w_sSQL & vbCrLf & "  and T15_GAKKA_CD (+)= T47_GAKKA_CD "
	w_sSQL = w_sSQL & vbCrLf & "  and T15_KAMOKU_CD (+)= T47_KAMOKU "
	
	If m_iGakunen <> "" Then
		w_sSQL = w_sSQL & vbCrLf & "  AND T47_KYOKASYO.T47_GAKUNEN=" & m_iGakunen
	End If
	
	if cstr(m_sSyozokuGakka) <> "00" then
	...
	w_sSQL = w_sSQL & vbCrLf & "  T15_SEQ_NO "
...
<%
		...
		if not f_ChkTantoKyokan(w_DispFlg,m_Rs("T47_GAKUNEN"),m_Rs("T47_GAKKA_CD"),m_Rs("T47_KAMOKU")) then exit sub
		
		if w_DispFlg then
		
		'If w_sCurGakkaCD = CStr(m_sSyozokuGakka) And m_sSyozokuGakka <> "00" Then
		'== 所属する学科の場合 ==
%>
<!-- <%= m_sGakka %> -->
...
'*	[引数]	
'********************************************************************************
Function f_ChkTantoKyokan(p_Flg,p_Gakunen,p_GakkaCd,p_KamokuCd)
	Dim wSql
	Dim wRs
	...
	wSql =  ""
	wSql = wSql & vbCrLf & " SELECT "
	wSql = wSql & vbCrLf & " 	* "
	wSql = wSql & vbCrLf & " FROM "
	wSql = wSql & vbCrLf & " 	T27_TANTO_KYOKAN "
	
	'if cstr(p_GakkaCd) <> cstr(C_CLASS_ALL) then
	if m_sSyozokuGakka <> "00" then
		wSql = wSql & vbCrLf & " 	,M05_CLASS "
	end if
	
	wSql = wSql & vbCrLf & " WHERE "
	
	'if cstr(p_GakkaCd) <> cstr(C_CLASS_ALL) then
	if m_sSyozokuGakka <> "00" then
		wSql = wSql & vbCrLf & " 	T27_NENDO = M05_NENDO and "
		wSql = wSql & vbCrLf & " 	T27_GAKUNEN = M05_GAKUNEN and "
		wSql = wSql & vbCrLf & " 	T27_CLASS = M05_CLASSNO and "
		wSql = wSql & vbCrLf & " 	M05_GAKKA_CD ='" & p_GakkaCd & "' and "
	end if
	
	wSql = wSql & vbCrLf & " 	T27_NENDO =" & m_iNendo & " and "
	wSql = wSql & vbCrLf & " 	T27_GAKUNEN =" & p_Gakunen & " and "
	wSql = wSql & vbCrLf & " 	T27_KAMOKU_CD ='" & p_KamokuCd & "' "
	
	If gf_GetRecordset(wRs,wSql) <> 0 Then Exit function
			...
			<Tr><Td>
				<table border="1" width="90%" class=hyo>
					...
					<Tr>
						...
						<input class=button type=button value="×削除" Onclick="f_Delete()" <%=m_Disabled%>></Td>
					</Tr>
				</table>
			</Td></Tr>
