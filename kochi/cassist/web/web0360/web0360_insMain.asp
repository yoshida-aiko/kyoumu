...
//		parent.topFrame.document.frm.txtTyuClubCd.disabled=false;
    ...
    function f_CheckData() {
		...
		document.frm.hidNyububi.value = "";
		obj = eval(document.frm.txtNyububi);
		...
		if(iMax==1){
			if(document.frm.GAKUSEI_NO.checked==false){
				...
				if(obj.value == ""){
					...
					if( chk_dateSplit(obj.value) == 1 ){
					    document.frm.txtNyububi.focus();
					    return 1;
					}
				}
				document.frm.hidNyububi.value = obj.value;
			}
		}else{
			...
			for (i = 0; i < iMax; i++) {
				if(document.frm.GAKUSEI_NO[i].checked==true){
					...
					if(document.frm.hidNyububi.value != ""){
						document.frm.hidNyububi.value = document.frm.hidNyububi.value + ",";
					}
					document.frm.hidNyububi.value = document.frm.hidNyububi.value + obj[i].value;
					w_bCheck = 0
				}
			};
    ...
    function f_detail(pGAKUSEI_NO){
			...
			url = "/catest/gak/gak0310/kojin.asp?hidGAKUSEI_NO=" + pGAKUSEI_NO;
			w   = 700;
			h   = 630;
			...
			<tr><td valign="top">
				...
				<table class=hyo border="1" bgcolor="#FFFFFF">
					...
					<tr>
						...
						<th nowrap class="header" width="130" align="center">入部日</th>
<!--
...
'//			w_iCnt = INT(m_iRsCnt/2 + 0.9)
			...
			Do until m_Rs.EOF 
				...
				%>
					<tr>
						...
						<td class="<%=w_Class%>" width="170" align="left" nowrap>
						...
						<td class="<%=w_Class%>" width="130" height="28">
							<% If gf_SetNull2String(m_Rs("T13_CLUB_1_FLG")) = "1" Then %>
								<div align="right"><%= m_Rs("T13_CLUB_1_NYUBI") %></div>
							<%Else%>
								<% If (gf_SetNull2String(m_Rs("T13_CLUB_1")) = m_sClubCd And gf_SetNull2String(m_Rs("T13_CLUB_1_FLG")) = "1") Or (gf_SetNull2String(m_Rs("T13_CLUB_2")) = m_sClubCd And gf_SetNull2String(m_Rs("T13_CLUB_2_FLG")) = "1") Then %>
									<BR>
								<% Else %>
									<div align="center"><input type="text" style="width:80px;" name="txtNyububi" id="id_Txt<%=wTbNo%>">&nbsp;<input type="button" class="button" onclick="fcalender('id_Txt<%=wTbNo%>')" value="選択"></div>
									<% wTbNo = wTbNo + 1 %>
								<% End If %>
							<% End If %>
						</td>
						...
						<tr>
						<td nowrap class="<%=w_Class%>" width="170" align="left" nowrap>
							<%
							'入部中なら表示する
						...
						<td class="<%=w_Class%>" width="130" height="28">
							<% If gf_SetNull2String(m_Rs("T13_CLUB_2_FLG")) = "1" Then %>
								<div align="right"><%= m_Rs("T13_CLUB_2_NYUBI") %></div>
							<%Else%>
								<% If (gf_SetNull2String(m_Rs("T13_CLUB_1")) = m_sClubCd And gf_SetNull2String(m_Rs("T13_CLUB_1_FLG")) = "1") Or (gf_SetNull2String(m_Rs("T13_CLUB_2")) = m_sClubCd And gf_SetNull2String(m_Rs("T13_CLUB_2_FLG")) = "1") Then %>
									<BR>
								<% Else %>
									<% If gf_SetNull2String(m_Rs("T13_CLUB_1_FLG")) <> "1" AND gf_SetNull2String(m_Rs("T13_CLUB_2_FLG")) <> "1" Then %>
										<BR>
									<% Else %>
										<div align="center"><input type="text" style="width:80px;" name="txtNyububi" id="id_Txt<%=wTbNo%>">&nbsp;<input type="button" class="button" onclick="fcalender('id_Txt<%=wTbNo%>')" value="選択"></div>
										<% wTbNo = wTbNo + 1 %>
	...
	<input type="hidden" name="hidNyububi">
