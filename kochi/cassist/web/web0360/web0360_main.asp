...
//				return 1;
    ...
    function f_detail(pGAKUSEI_NO){
			...
			//2014/07/28 UPDATE KIYOMOTO
			//url = "/cassist/gak/gak0300/kojin.asp?hidGAKUSEI_NO=" + pGAKUSEI_NO;
			url = "../../gak/gak0310/kojin.asp?hidGAKUSEI_NO=" + pGAKUSEI_NO;
			//2014/07/28 UPDATE END
			w   = 700;
			h   = 630;
			...
			%>
				<tr>
					...
					<%If m_bKomon = True Then%>
						...
						<input type="button" class="button" onclick="fcalender('id_Txt1<%=i-1%>')" value="選択"> <!--2014/08/07 UPDATE URAKAWA onclick="fcalender('id_Txt2<%=i-1%>')　→　onclick="fcalender('id_Txt1<%=i-1%>')　に変更-->
					<%Else%>
						<td nowrap class="<%=w_Class%>" align="center"  >
						<%=w_NyuBi%>&nbsp;
					<%End If%>
