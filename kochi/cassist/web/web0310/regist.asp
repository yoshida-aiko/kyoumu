...
' システム名: 教務事務システム
' 処  理  名: 時間割交換連絡
' ﾌﾟﾛｸﾞﾗﾑID : web/web0310/regist.asp
' 機      能: 上ページ 表示情報を表示
'-------------------------------------------------------------------------
...
Dim w_rs
    ...
    Do
        ...
        m_sSQL = m_sSQL & " T52_NAIYO,T52_KAISI,T52_SYURYO "
        m_sSQL = m_sSQL & "FROM "
        ...
        '//取得した値を変数に代入
        m_sNaiyou   = w_rs("T52_NAIYO")
        m_sKaisibi  = w_rs("T52_KAISI")
        m_sSyuryoubi= w_rs("T52_SYURYO")
...
<%End If%>
    ...
    function f_Touroku(){
        ...
        //リスト情報をsubmit
        document.frm.target = "<%=C_MAIN_FRAME%>" ;
        document.frm.action = "web0310_edt.asp";
        document.frm.submit();
    ...
    function f_CheckData() {
        ...
        // ■ 完了日
        if(document.frm.Syuryoubi.value != "" ){
	...
	<% If m_stxtMode = "NEW" or m_stxtMode = "NEW2" Then 
	    call gs_title("時間割交換連絡","新　規")
	   Else
	    call gs_title("時間割交換連絡","修　正")
	   End If%>
	...
	<br>
	<table class=disp width="510"><tr>
	    <td class=disp>
	    <span class=rule><br>
	    下記の内容を参考にして、授業の変更内容を記述して下さい。 <br>
	    例 )
	    〇月〇日〇時限、〇〇先生の〇〇と、Δ月Δ日Δ時限、ΔΔ先生のΔΔが入れ替わりました。 <br><br>
	    </span>
	    </td>
	</tr></table><br>
	...
	%>
		...
		<input type="button" value="一覧へ" class=button onclick="javascript:f_itiran()">
