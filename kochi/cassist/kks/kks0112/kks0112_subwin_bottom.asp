...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Public m_AryJigen()			'//ヘッダ情報格納配列
	Public m_AryState()			'//欠課･遅刻数セット配列
...
Sub Main()
    ...
    Do
		...
		gEnd = Timer '検索時間を表示する為
		'response.write "検索時間=" & Round(gEnd - gStart,3) & "秒<BR>"
...
function f_HeadData()
	...
	m_JigenCount = gf_GetRsCount(w_Rs)
	...
	for w_num = 0 to m_JigenCount-1
		...
		m_AryJigen(w_num,0) = day(w_Rs("T21_HIDUKE"))
		m_AryJigen(w_num,1) = w_Rs("T21_JIGEN")
		m_AryJigen(w_num,2) = w_Rs("T21_HIDUKE")
...
function f_Get_KekkaData()
	...
	If gf_GetRecordset(w_Rs_Student,w_sSQL) <> 0 Then
		'ﾚｺｰﾄﾞｾｯﾄの取得失敗
		msMsg = Err.description
		exit function
	End If
	
	m_StudentCount = gf_GetRsCount(w_Rs_Student)		'//生徒数
	...
	w_AryXCount = 2 + m_JigenCount						'//(学籍NO+生徒氏名) + 時限数
	
	ReDim Preserve m_AryState(w_AryXCount,m_StudentCount)	'//欠課･遅刻数セット配列
	
	for w_num = 0 to m_StudentCount
		
		m_AryState(0,w_num) = w_Rs_Student(0)		'学籍NO
		m_AryState(1,w_num) = w_Rs_Student(1)		'生徒氏名
		
		for w_Jnum = 0 to m_JigenCount - 1
			'w_Jnum時限の欠課区分
			if not f_SetKekka(w_Rs_Student(0),m_AryJigen(w_Jnum,1),m_AryJigen(w_Jnum,2),m_AryState(2+w_Jnum,w_num)) then exit function
		next
		
		w_Rs_Student.movenext
		
	next
	
	'// 終了処理
    Call gf_closeObject(w_Rs_Student)
