...
Function f_GetSubject()
	...
	w_sSQL = w_sSQL & "		,T27_KAMOKU_KBN as KAMOKU_KBN "
	w_sSQL = w_sSQL & "from"
	...
	w_sSQL = w_sSQL & "	and	T27_KAMOKU_KBN = " & C_JIK_JUGYO
	...
	w_sSQL = w_sSQL & "	and	T27_KAMOKU_KBN = " & C_JIK_TOKUBETU
	...
	iRet = gf_GetRecordset(m_Rs_Subject,w_sSQL)
...
%>
    ...
    function f_Search(){
		...
		//url = url + "&hidClassName=" + vl[4];
		//url = url + "&hidKamokuName=" + vl[5];
		...
		//document.frm.target="kks0112_subwin";
		//document.frm.action="kks0112_subwin.asp";
		//document.frm.submit();
    }
							...
							<select name="sltKamoku" style="width:200px;">
								<% 
									do until m_Rs_Subject.EOF
										...
										w_Syubetu = ""
										if cint(m_Rs_Subject("KAMOKU_KBN")) = C_JIK_JUGYO then
											w_Syubetu = "TUJO"  '//通常授業
										else
											w_Syubetu = "TOKU"  '//特別活動
										end if
								%>
										<option value="<%=CStr(w_Syubetu & "#@#" & m_Rs_Subject("KAMOKU_CD") & "#@#" & m_Rs_Subject("GAKUNEN") & "#@#" & m_Rs_Subject("CLASS") & "#@#" & w_ClassName & "#@#" & m_Rs_Subject("KAMOKU_NAME"))%>"><%=m_Rs_Subject("GAKUNEN") & "年&nbsp;&nbsp;" & w_ClassName & "&nbsp;&nbsp;&nbsp;" & m_Rs_Subject("KAMOKU_NAME") %>
								<%
