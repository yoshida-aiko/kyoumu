...
'Call s_DebugPrint()
		...
		'// 管理マスタより、出欠欠課の取り方を取得
		'w_iRet = f_GetKanriInfo(m_iSyubetu)
		...
		Else
			...
			If m_Rs_M.EOF Then
				'//空白ページ表示
				Call showWhitePage("履修データがありません")
				Exit Do
			End If
...
Function f_Get_HeadData()
	...
	Do 
		...
		w_sSQL = w_sSQL & vbCrLf & "  B.T20_JIGEN,"
		w_sSQL = w_sSQL & vbCrLf & "  B.T20_YOUBI_CD"
...
'		 w_sSQL = w_sSQL & vbCrLf & " AND B.T20_JIGEN = A.T32_JIGEN"
		...
		If w_Rs.EOF = false Then
			...
			Do Until w_Rs.EOF
				...
				iRet = f_Get_DateInfo(w_Rs("T32_HIDUKE"),cint(w_Rs("T20_JIGEN")),w_bGyoji)
				If iRet <> 0 Then
					msMsg = Err.description
					f_Get_HeadData = 99
					Exit Do
				...
				If w_bGyoji <> True Then
					...
					Else
						...
						m_AryHead(2,i) = gf_SetNull2String(w_Rs("T20_YOUBI_CD"))		  '//曜日CD
					End If

					'//小数点を文字に変更する
					m_AryHead(3,i) = replace(gf_SetNull2String(w_Rs("T20_JIGEN")),".","$")	'//時限
...
'response.write w_sSQL & "<br>"
		...
		'===================================================
		'//レベル課目ﾌﾗｸﾞを判別(C_LEVEL_YES=1:レベル別課目,C_LEVEL_NO=0:通常科目)
		'===================================================
		'If cstr(trim(w_sLevelFlg)) = cstr(C_LEVEL_YES) Then

'response.write "<font color=#000000>通常授業　<<<レベル別>>>　　START" & "<br>"

			'//レベル別課目生徒一覧取得
		 '	 w_iRet = f_Get_TUJO_LevelBetu()
		 '	 If w_iRet <> 0 Then
		 '		 Exit Do
		 '	 End If
		'Else
'response.write "<font color=#000000>通常授業　<<<通常>>>　　START" & "<br>"
...
'response.write "<font color=#000000> <<<レベル別ﾚｺｰﾄﾞ>>></font><br>"
...
Function f_Get_TUJO_Tujyo()
	...
	Do 
		...
		w_sSQL = w_sSQL & vbCrLf & " WHERE "
		'w_sSQL = w_sSQL & vbCrLf & "   T13_GAKU_NEN.T13_NENDO - T13_GAKU_NEN.T13_GAKUNEN + 1 = T11_GAKUSEKI.T11_NYUNENDO AND "
...
'response.write "<font color=#000000> <<<通常ﾚｺｰﾄﾞ>>></font><br>"
...
Function f_Get_Data_KOBETU()
	...
	Do 
		...
		w_sSQL = w_sSQL & vbCrLf & " WHERE "
		'w_sSQL = w_sSQL & vbCrLf & "   T11_GAKUSEKI.T11_NYUNENDO = T13_GAKU_NEN.T13_NENDO - T13_GAKU_NEN.T13_GAKUNEN + 1 AND "
...
'response.write "個別授業" & "<br>" & w_sSQL & "<BR>"
		iRet = gf_GetRecordset(m_Rs_M, w_sSQL)
		If iRet <> 0 Then
			'ﾚｺｰﾄﾞｾｯﾄの取得失敗
			msMsg = Err.description
...
Function f_Get_TUJO_DaigeRyugak()
	...
	Do 
		...
		w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_NENDO="		& cInt(m_iSyoriNen) & " AND "
		w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_GAKKI_KBN='" & m_sGakki_Kbn		& "' AND "
		'w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_GAKUNEN="	& cInt(m_sGakunen)	& " AND "
		'w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_CLASS=" 	& cInt(m_sClassNo)	& " AND "
		w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_KAMOKU='"	& m_sKamokuCd		& "' AND "
		w_sSQL = w_sSQL & vbCrLf & "  T23_DAIGAE_JIKAN.T23_KYOKAN='"	& m_iKyokanCd		& "'"
		w_sSQL = w_sSQL & vbCrLf & " ORDER BY GAKUSEKI"
...
'		w_sSQL = w_sSQL & vbCrLf & "  AND T24_SIKEN_NITTEI.T24_JISSI_SYURYO>='" & gf_YYYY_MM_DD(m_iSyoriNen & "/" & month(date()) & "/" & day(date()),"/")	& "'"
		w_sSQL = w_sSQL & vbCrLf & "  AND T24_SIKEN_NITTEI.T24_JISSI_SYURYO>='" & w_sDate & "'"
'		w_sSQL = w_sSQL & vbCrLf & " ORDER BY T24_SIKEN_NITTEI.T24_SIKEN_KBN DESC"
...
Function f_Get_AbsInfo_RuiKei()
	...
	Do 
		...
		w_bRtn = gf_GetKaisiSyuryo(cint(w_iSiken), m_sGakunen, w_sKaisibi, w_sSyuryobi)
		If w_bRtn <> True Then
			Exit Function
		End If

'response.write "w_sKaisibi=" & w_sKaisibi & "<BR>"
'response.write "w_sSyuryobi=" & w_sSyuryobi & "<BR>"
'
''//テスト
'w_sKaisibi="2001/04/01"
'w_sSyuryobi="2001/08/31"
'm_iSikenKbn=1
'w_iSiken=1


		'//後期以降の場合は、後期期末試験終了日から、本日までの期間の出欠を取得
		If w_iSiken = 4 Then
'			w_sKaisibi = w_sSyuryobi
			w_sSyuryobi = gf_YYYY_MM_DD(m_iSyoriNen & "/" & month(date()) & "/" & day(date()),"/")
		Else
			w_sSyuryobi = gf_YYYY_MM_DD(m_iSyoriNen & "/" & month(date()) & "/" & day(date()),"/")
		End If
...
'
'		 If m_sGakki = "ZENKI" Then
'			 w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE >= '" & m_sZenki_Start & "' AND "
'			 w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE <  '" & m_sKouki_Start & "' AND "
'		 Else
'			 w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE >= '" & m_sKouki_Start & "' AND "
'			 w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE <= '" & m_sKouki_End	 & "' AND "
'		 End If

		w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE >= '" & w_sKaisibi & "' AND "
		w_sSQL = w_sSQL & vbCrLf & "    A.T21_HIDUKE <  '" & w_sSyuryobi & "' AND "
...
'				 If cstr(rs("T21_SYUKKETU_KBN"))=cstr(C_KETU_KEKKA) Then
'					 'm_AryRuiKei(1,i) = rs("CNT")	  '//欠課数
'					 m_AryRuiKei(1,i) = rs("KAISU")    '//欠課数
'
'				 Else
'					If cstr(rs("T21_SYUKKETU_KBN"))=cstr(C_KETU_TIKOKU) Then
'						'm_AryRuiKei(2,i) = rs("CNT")	 '//遅刻数
'						m_AryRuiKei(2,i) = rs("KAISU")	  '//遅刻数
'					End If
'				 End If
...
'				 If cstr(rs("T21_SYUKKETU_KBN"))=cstr(C_KETU_KEKKA) Then
'					 m_AryTukiKei(1,i) = rs("CNT")	 '//欠課数
'				 Else
'					 m_AryTukiKei(2,i) = rs("CNT")	 '//遅刻数
'				 End If
...
'	For j=0 to 2
'		response.write "m_AryTukiKei(" & j & "," & i & ")=" & m_AryTukiKei(j,i) & "<br>"
'	Next
'response.write "<br>"
'Next
...
'response.write "2選択フラグ= " & gf_SetNull2Zero(m_Rs_M("T16_SELECT_FLG")) & " = " & cint(C_SENTAKU_NO) & "::" & w_bNoSelect
					...
					For i = 0 to m_iRsCnt
						...
						Else
'response.write "ccc" & w_bNoChange & "aaa"
							...
							Else%>
								...
								<% '変更可能期間の場合 %>
								<% if m_sEndDay < m_AryHead(4,i) then 
									w_bEndFLG = False %>
								...
								<% Else %>
									<td class="<%=w_Class%>" align="center" width="30" height="28" nowrap>
									<input type="button" class="<%=w_Class%>" name="KBN<%=m_Rs_M("GAKUSEI") & "_" & replace(m_AryHead(4,i),"/","") & "_" & m_AryHead(3,i)%>" size="2" value="<%=w_Syuketu_R%>"	style="border-style:none" style="text-align:center" tabindex="-1">
									<input type="hidden" name="hidKBN<%=m_Rs_M("GAKUSEI") & "_" & replace(m_AryHead(4,i),"/","") & "_" & m_AryHead(3,i)%>" size="2" value="<%=w_Syuketu%>"></td>
								<%End If%>
					...
					Call gs_cellPtn(w_Class) 
						...
						for i = 0 to m_iRsCnt
							...
							If w_bNoChange = False Then
								'If cstr(m_AryHead(3,i)) <> cstr(m_Rs_D("T23_JIGEN")) Then
							...
							<%Else%>
								...
								<% Else %>
									<td class="<%=w_Class%>" align="center"  width="30" height="28"  nowrap>
									<input type="button" class="<%=w_Class%>" name="KBN<%=m_Rs_D("GAKUSEI") & "_" & replace(m_AryHead(4,i),"/","") & "_" & m_AryHead(3,i)%>" size="2" value="<%=w_Syuketu_R%>" style="border-style:none" style="text-align:center" tabindex="-1">
									<input type="hidden" name="hidKBN<%=m_Rs_D("GAKUSEI") & "_" & replace(m_AryHead(4,i),"/","") & "_" & m_AryHead(3,i)%>" size="2" value="<%=w_Syuketu%>"></td>
								<%End If%>
...
'								Call gf_GetKekaChi(m_iSyoriNen,m_iSikenKbn,m_sKamokuCd,w_sGakusekiNo,p_iKekka,p_iChikoku,w_iKekkaGai)
					...
					%>
<!--<%= m_AryRuiKei(1,i) & ":" & m_AryRuiKei(2,i) %>-->
						...
						For i=0 To m_iRuiKeiCnt
							If w_sGakusekiNo=m_AryRuiKei(0,i) Then
								...
								If m_iSyubetu = C_K_KEKKA_RUISEKI_KEI Then
									...
									'//前回の試験時の出欠を取得する
									w_sGakusekiNo = m_Rs_D("GAKUSEI")		'2001/12/17 Add
'									Call gf_GetKekaChi(m_iSyoriNen,m_iSikenKbn,m_sKamokuCd,w_sGakusekiNo,p_iKekka,p_iChikoku,w_iKekkaGai)
									Call gf_GetKekaChi(m_iSyoriNen,w_iKbn,m_sKamokuCd,w_sGakusekiNo,p_iKekka,p_iChikoku,w_iKekkaGai)
									w_RuiKekka = cint(w_RuiKekka) + cint(gf_SetNull2Zero(p_iKekka))
									w_RuiTikoku = cint(w_RuiTikoku) + cint(gf_SetNull2Zero(p_iChikoku))
