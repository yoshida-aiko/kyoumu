...
Function f_GetKamoku()
    ...
    Do 
		...
		'試験時間割からレコードセットが取れなかった場合は、担当教官Tからコンボ作成
			...
			w_sSQL = w_sSQL & vbCrLf & "    AND M05_GAKKA_CD(+) = T16_GAKKA_CD "
								...
								<%Else%>
									...
									<%Else%>
										...
										<%Do Until m_Rs.EOF%>
											...
											%>
											<option value="<%=m_Rs("GAKUNEN") & "#@#" & m_Rs("CLASS") & "#@#" & m_Rs("KAMOKU") & "#@#" & m_Rs("GAKKA_CD") & "#@#" & gf_GetT16UpdDate(m_iNendo,m_Rs("GAKUNEN"),m_Rs("GAKKA_CD"),m_Rs("KAMOKU"),"") & "#@#" & w_TukuName%>"><%=w_Str%>
