...
Function f_GetRyuDaigae(p_sikenKBN)
    ...
    Do
        ...
        w_sSQL = ""
        w_sSQL = w_sSQL & vbCrLf & " SELECT "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_GAKUNEN, "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_CLASS, "
        w_sSQL = w_sSQL & vbCrLf & " M05.M05_CLASSMEI, "
        w_sSQL = w_sSQL & vbCrLf & " M05.M05_GAKKA_CD, "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_KAMOKU_CD, "
        w_sSQL = w_sSQL & vbCrLf & " T16.T16_OKIKAE_FLG, "
        w_sSQL = w_sSQL & vbCrLf & " T16.T16_KAMOKUMEI "
        w_sSQL = w_sSQL & vbCrLf & " FROM "
        w_sSQL = w_sSQL & vbCrLf & " T16_RISYU_KOJIN T16,T27_TANTO_KYOKAN T27 ,M05_CLASS M05"
IF p_sikenKBN <> cint(C_SIKEN_KOU_KIM) THEN
        w_sSQL = w_sSQL & vbCrLf & " ,T23_DAIGAE_JIKAN T23"
END IF
        w_sSQL = w_sSQL & vbCrLf & " WHERE "
		w_sSQL = w_sSQL & vbCrLf & " T27.T27_SEISEKI_INP_FLG =" & C_SEISEKI_INP_FLG_YES & " AND "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_KYOKAN_CD 	= '"	&	m_sKyokanCd			&	"' AND "
'        w_sSQL = w_sSQL & vbCrLf & " T16.T16_OKIKAE_FLG 	= "		&	C_TIKAN_KAMOKU_SAKI	&	"  AND "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_NENDO		= "		& 	cInt(m_iNendo) 	& 	"  AND "
'        w_sSQL = w_sSQL & vbCrLf & " T27.T27_GAKKI_KBN	= '" 	& 	m_iGakki 		& 	"' AND "

IF p_sikenKBN <> cint(C_SIKEN_KOU_KIM) THEN
        w_sSQL = w_sSQL & vbCrLf & " T23.T23_GAKKI_KBN		= " & w_sGakkiKbn & " AND "
        w_sSQL = w_sSQL & vbCrLf & " T23.T23_NENDO 		= T27.T27_NENDO AND "
        w_sSQL = w_sSQL & vbCrLf & " T23.T23_KAMOKU		= T27.T27_KAMOKU_CD AND "
        w_sSQL = w_sSQL & vbCrLf & " T23.T23_KYOKAN		= T27.T27_KYOKAN_CD AND "
END IF
        w_sSQL = w_sSQL & vbCrLf & " T16.T16_KAMOKU_CD 	= T27.T27_KAMOKU_CD AND "
        w_sSQL = w_sSQL & vbCrLf & " T16.T16_NENDO 		= T27.T27_NENDO AND"
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_NENDO 		= M05.M05_NENDO AND "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_GAKUNEN		= M05.M05_GAKUNEN AND "
        w_sSQL = w_sSQL & vbCrLf & " T27.T27_CLASS 		= M05.M05_CLASSNO "
        w_sSQL = w_sSQL & vbCrLf & " GROUP BY T27.T27_GAKUNEN,T27.T27_CLASS,M05.M05_CLASSMEI, "
        w_sSQL = w_sSQL & vbCrLf & " M05.M05_GAKKA_CD,T27.T27_KAMOKU_CD,T16.T16_OKIKAE_FLG,T16.T16_KAMOKUMEI "

        w_sSQL = w_sSQL & vbCrLf & " ORDER BY T16_OKIKAE_FLG DESC "
...
//alert("aaaaaa" + document.frm.cboKamoku.value + "aa")
								...
								<%Else%>
									...
									<%Else%>
										...
										<%Do Until m_Rs.EOF%>
											...
											%>
											<option value="<%=m_Rs("GAKUNEN") & "#@#" & m_Rs("CLASS") & "#@#" & m_Rs("KAMOKU") & "#@#" & m_Rs("GAKKA_CD") & "#@#" & gf_GetT16UpdDate(m_iNendo,m_Rs("GAKUNEN"),m_Rs("GAKKA_CD"),m_Rs("KAMOKU"),"")%>"><%=w_Str%>
										...
										<%Do Until m_Rs_Ryu.EOF%>
											<%
												...
												%>
												<option value="<%=m_Rs_Ryu("T27_GAKUNEN") & "#@#" & m_Rs_Ryu("T27_CLASS") & "#@#" & m_Rs_Ryu("T27_KAMOKU_CD") & "#@#" & m_Rs_Ryu("M05_GAKKA_CD") & "#@#" & gf_GetT16UpdDate(m_iNendo,m_Rs_Ryu("T27_GAKUNEN"),m_Rs_Ryu("M05_GAKKA_CD"),m_Rs_Ryu("T27_KAMOKU_CD"),"")%>"><%=w_Str%>
