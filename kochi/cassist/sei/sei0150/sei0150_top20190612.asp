...
' 変      更: 2019/06/11 藤林　高知高専　留年生などクラスのメイン学科ではない生徒への対応のため、科目コンボの作成方法修正
'*************************************************************************/
%>
...
Function f_GetSubject()
	...
	'UPD 2019/06/12 Fujibayashi
	''UPD 2019/06/11 Fujibayashi
	...
	''クラスマスタのメイン学科以外のみ受講する場合、学科略称を表示、それ以外はクラス名を表示
	'w_sSQL = w_sSQL & "		,DECODE(SUM(CASE WHEN M05_GAKKA_CD = T15_GAKKA_CD THEN 1 ELSE 0 END) , 0 , MAX(M02_GAKKARYAKSYO) , MAX(M05_CLASSMEI) ) AS CLASS_NAME "
	''クラスマスタのメイン学科以外のみ受講する場合、カリキュラムの学科コードを表示、それ以外はメインの学科コードを表示
	'w_sSQL = w_sSQL & "		,DECODE ( SUM ( CASE WHEN M05_GAKKA_CD = T15_GAKKA_CD THEN 1 ELSE 0 END ) , 0 , MAX ( T15_GAKKA_CD ) , M05_GAKKA_CD ) AS GAKKA_CD "
	''UPD 2019/06/11 End
    '
	'w_sSQL = w_sSQL & " from"
	...
	'w_sSQL = w_sSQL & "		,M02_GAKKA "				'ADD 2019/06/11 Fujibayashi
	'w_sSQL = w_sSQL & " where "
	...
	'w_sSQL = w_sSQL & "	and  T15_COURSE_CD IN ( '0' , CASE WHEN M05_GAKKA_CD = T15_GAKKA_CD THEN (CASE WHEN M05_COURSE_CD IS NOT NULL THEN M05_COURSE_CD ELSE T15_COURSE_CD END ) ELSE T15_COURSE_CD END ) "
	''UPD END 2019/06/11
    '
    ...
    '
	'w_sSQL = w_sSQL & " and T27_NENDO = M02_NENDO "				'ADD 2019/06/11 Fujibayashi
	'w_sSQL = w_sSQL & " and T15_GAKKA_CD = M02_GAKKA_CD "		'ADD 2019/06/11 Fujibayashi
    '
	...
	'w_sSQL = w_sSQL & "        M03_KAMOKU_KBN , "
	'w_sSQL = w_sSQL & "        T27_KAMOKU_BUNRUI, "
	...
	w_sSQL = w_sSQL & "	  , M03_KAMOKU_KBN AS KAMOKU_KBN_IS "
	w_sSQL = w_sSQL & "	  , T27_KAMOKU_BUNRUI AS KAMOKU_KBN "
	w_sSQL = w_sSQL & "	  , DECODE ( MAIN_CLASS ,'1' , M05_CLASSMEI, M02_GAKKARYAKSYO ) AS CLASS_NAME "
	w_sSQL = w_sSQL & "	  , T13_GAKKA_CD "
	w_sSQL = w_sSQL & "FROM  ( "
	w_sSQL = w_sSQL & "		   SELECT  T27_NENDO "
	w_sSQL = w_sSQL & "				 , T27_GAKUNEN "
	w_sSQL = w_sSQL & "				 , T27_CLASS "
	w_sSQL = w_sSQL & "				 , T27_KAMOKU_CD "
	w_sSQL = w_sSQL & "				 , T27_KAMOKU_BUNRUI "
	...
	w_sSQL = w_sSQL & "					  , T27_KAMOKU_BUNRUI "
	w_sSQL = w_sSQL & "					  , T13_GAKKA_CD "
	w_sSQL = w_sSQL & "					  , CASE WHEN MGAKKA.M05_NENDO IS NULL THEN '0' "
	w_sSQL = w_sSQL & "							 ELSE T13_COURCE_CD END AS COURSE_CD "
	w_sSQL = w_sSQL & "					  , CASE WHEN MGAKKA.M05_NENDO IS NULL THEN '0' "
	w_sSQL = w_sSQL & "							 ELSE '1' END AS MAIN_CLASS "
	w_sSQL = w_sSQL & "				FROM "
	w_sSQL = w_sSQL & "					  T27_TANTO_KYOKAN "
	w_sSQL = w_sSQL & "					, T13_GAKU_NEN "
	w_sSQL = w_sSQL & "					, M05_CLASS MGAKKA "
	w_sSQL = w_sSQL & "					, T15_RISYU "
	w_sSQL = w_sSQL & "				WHERE "
	w_sSQL = w_sSQL & "					  T27_NENDO =" & cint(m_iNendo)
	w_sSQL = w_sSQL & "				  AND T27_KYOKAN_CD ='" & m_sKyokanCd & "'"
	w_sSQL = w_sSQL & "				  AND T27_KAMOKU_BUNRUI = " & C_JIK_JUGYO
	w_sSQL = w_sSQL & "				  AND T27_SEISEKI_INP_FLG = " & C_SEISEKI_INP_FLG_YES
	w_sSQL = w_sSQL & "				  AND T27_NENDO = T13_NENDO "
	w_sSQL = w_sSQL & "				  AND T27_GAKUNEN = T13_GAKUNEN "
	w_sSQL = w_sSQL & "				  AND T27_CLASS = T13_CLASS "
	w_sSQL = w_sSQL & "				  AND T13_NENDO = MGAKKA.M05_NENDO(+) "
	w_sSQL = w_sSQL & "				  AND T13_GAKUNEN = MGAKKA.M05_GAKUNEN(+) "
	w_sSQL = w_sSQL & "				  AND T13_CLASS = MGAKKA.M05_CLASSNO(+) "
	w_sSQL = w_sSQL & "				  AND T13_GAKKA_CD= MGAKKA.M05_GAKKA_CD(+) "
	w_sSQL = w_sSQL & "				  AND T15_NYUNENDO = T13_NENDO - T13_GAKUNEN + 1 "
	w_sSQL = w_sSQL & "				  AND T15_KAMOKU_CD = T27_KAMOKU_CD "
	w_sSQL = w_sSQL & "				  AND T15_GAKKA_CD = T13_GAKKA_CD "
	w_sSQL = w_sSQL & "				  AND T15_COURSE_CD IN ( '0' , T13_COURCE_CD ) "
	...
	w_sSQL = w_sSQL & "	  ) "
	w_sSQL = w_sSQL & "	  , M03_KAMOKU "
	w_sSQL = w_sSQL & "	  , M02_GAKKA "
	w_sSQL = w_sSQL & "	  , M05_CLASS "
	w_sSQL = w_sSQL & "WHERE "
	w_sSQL = w_sSQL & "	  T27_NENDO = M03_NENDO "
	w_sSQL = w_sSQL & "  AND T27_KAMOKU_CD = M03_KAMOKU_CD "
	w_sSQL = w_sSQL & "  AND T27_NENDO = M02_NENDO "
	w_sSQL = w_sSQL & "  AND T13_GAKKA_CD = M02_GAKKA_CD "
	w_sSQL = w_sSQL & "  AND T27_NENDO = M05_NENDO "
	w_sSQL = w_sSQL & "  AND T27_GAKUNEN = M05_GAKUNEN "
	w_sSQL = w_sSQL & "  AND T27_CLASS = M05_CLASSNO "
	'UPD 2019/06/12 Fujibayashi
	...
	'UPD 2019/06/11 Fujibayashi
	'w_sSQL = w_sSQL & " 	,M05_CLASSMEI AS CLASS_NAME "
	w_sSQL = w_sSQL & "		,CASE 	WHEN M05_GAKKA_CD = T16_GAKKA_CD THEN M05_CLASSMEI ELSE M02_GAKKARYAKSYO END AS CLASS_NAME "
	'UPD 2019/06/11 End
	...
	'2019/06/11 Upd Fujibayashi
	'w_sSQL = w_sSQL & "    AND M05_GAKKA_CD(+) = T16_GAKKA_CD "
	w_sSQL = w_sSQL & "    AND T16_COURSE_CD IN ( '0' , CASE WHEN M05_GAKKA_CD = T16_GAKKA_CD THEN (CASE WHEN M05_COURSE_CD IS NOT NULL THEN M05_COURSE_CD ELSE T16_COURSE_CD END ) ELSE T16_COURSE_CD END ) "
	'2019/06/11 Upd End
	...
	w_sSQL = w_sSQL & "    AND T27_NENDO = M02_NENDO "			'ADD 2019/06/11 Fujibayashi
	w_sSQL = w_sSQL & "    AND T16_GAKKA_CD = M02_GAKKA_CD "	'ADD 2019/06/11 Fujibayashi
	...
	'UPD 2019/06/11 Fujibayashi
	'w_sSQL = w_sSQL & "		,M05_CLASSMEI as CLASS_NAME "
	w_sSQL = w_sSQL & "		,CASE 	WHEN M05_GAKKA_CD = T15_GAKKA_CD THEN M05_CLASSMEI ELSE M02_GAKKARYAKSYO END AS CLASS_NAME "
	'UPD 2019/06/11 End
	...
	w_sSQL = w_sSQL & " and T26_NENDO = M02_NENDO "			'ADD 2019/06/11 Fujibayashi
	w_sSQL = w_sSQL & " and T15_GAKKA_CD = M02_GAKKA_CD "	'ADD 2019/06/11 Fujibayashi
...
%>
	...
	function f_SetData(){
		...
		document.frm.txtClassMei.value=vl[7];		//2019/06/11 Add Fujibayashi
		document.frm.txtKamokuMei.value=vl[8];		//2019/06/11 Add Fujibayashi
	}
								...
								<%
									if gRs.EOF Then
										...
										do until gRs.EOF
											...
											w_SubjectValue = w_SubjectValue & "#@#" & gRs("CLASS_NAME")			'2019/06/11 Add Fujibayashi
											w_SubjectValue = w_SubjectValue & "#@#" & gRs("KAMOKU_NAME")		'2019/06/11 Add Fujibayashi
								%>
	...
	<input type="hidden" name="txtClassMei"   value="">		<%	'2019/06/11 Add Fujibayashi	%>
	<input type="hidden" name="txtKamokuMei"   value="">	<%	'2019/06/11 Add Fujibayashi	%>
