...
'-------------------------------------------------------------------------
' 作      成: 2002/06/21 shin
' 作      成: 2005/04/26 西村 沼津高専用に公休欄追加
' 作      成: 2005/06/13 西村 福島高専用に授業時間数表示処理追加
' 作      成: 2017/12/12 西村 一般、指定科目は　学科表示の場合、140050生物は除外する
' 変      更: 2019/06/11 藤林　高知高専　留年生などクラスのメイン学科ではない生徒への対応(クラス名に学科名を表示する)
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Dim m_sClassMei		'クラス名(留年生などクラスのメイン学科ではない生徒のみの受講科目は学科名)	'2019/06/11 Add Fujibayashi
	Dim m_sKamokuMei		'クラス名(留年生などクラスのメイン学科ではない生徒のみの受講科目は学科名)	'2019/06/11 Add Fujibayashi
...
Sub Main()
	...
	Do
	    ...
	    If not f_GetKongoClass(m_iGakunen,cint(m_sClassNo),m_iKongou) Then m_bErrFlg = True : Exit Do

		If cint(m_iKamokuKbn) = cint(C_JIK_JUGYO) then  '通常授業の場合
		  '//科目情報を取得
...
Sub s_SetParam()
	...
	m_sClassMei =request("txtClassMei")		'2019/06/11 Add Fujibayashi
	m_sKamokuMei =request("txtKamokuMei")	'2019/06/11 Add Fujibayashi
...
Function f_GetKamokuName(p_Gakunen,p_GakkaCd,p_KamokuCd)
	...
	'2019/06/11 Upd Fujibayashi(科目名はコンボの値から引き継ぐように変更)
	'Dim w_sSQL
	'Dim w_Rs
	'Dim w_GakkaCd
...
<!--
	...
	function setHyoka(){
		...
		for(w_num=1;w_num<<%=m_iCount%>;w_num++){
			...
			// 20040218 修正 shiki
			// 沼津で免除FLG(parent.main.document.frm.hidMenjiFlg)が "1" の場合、
			// 未評価チェックボックスをクリックされても何もしない
			ob2 = eval("parent.main.document.frm.hidMenjiFlg" + w_num);
			...
			if (typeof(ob2) == 'object'){
				...
				if(ob2.value == "1"){
					...
					//選択科目で履修していない学生の評価不能フラグを設定
					//2004.02.24 ITO
					if(typeof(ob[0]) != "undefined" && ob[0].type == "hidden"){
						if(w_type){
							ob[0].value="4"
						}else{
							ob[0].value=""
	...
	<table border="0" cellpadding="0" cellspacing="0" height="245" width="100%">
					...
					<tr>
						...
						if m_iKongou <> C_CLASS_KONGO Then
							...
							if Cint(m_iKamoku_Kbn) <> C_KAMOKU_SENMON Then
								...
								'UPP 2017/12/12 Nishimura  OR (m_sKamokuCd = 140050 )を除外
								If m_sGakkoNO = C_NCT_KOCHI Then
									If (m_sKamokuCd = 140046 ) OR (m_sKamokuCd = 140047 ) OR (m_sKamokuCd = 140048 ) OR (m_sKamokuCd = 180011 ) OR (m_sKamokuCd = 180012 ) OR (m_sKamokuCd = 180013 ) OR (m_sKamokuCd = 180014 ) Then
										w_str = m_iGakunen & "年　" & gf_GetGakkaNm(m_iNendo,m_sGakkaCd) & "　" & f_GetKamokuName(m_iGakunen,m_sGakkaCd,m_sKamokuCd)
									Else
										'2019/06/11 Upd Fujibayashi
										'w_str = m_iGakunen & "年　" & gf_GetClassName(m_iNendo,m_iGakunen,m_sClassNo) & "　" & f_GetKamokuName(m_iGakunen,m_sGakkaCd,m_sKamokuCd)
										w_str = m_iGakunen & "年　" & m_sClassMei & "　" & f_GetKamokuName(m_iGakunen,m_sGakkaCd,m_sKamokuCd)
										'2019/06/11 Upd End
									End If
								Else
		...
		<tr>
			<td align="center" valign="bottom" nowrap>
				...
				<%  ' INS福島の場合　授業時間数表示　INS2005/06/13 西村
					if m_sGakkoNO = C_NCT_FUKUSHIMA then
				%>
					...
					<tr>
						<th class="header3" colspan="14" nowrap align="center">
							[前中]&nbsp;総授業数<%= Request("hidSouJyugyou_TZ") %>
 									純授業数<%= Request("hidJunJyugyou_TZ") %>
								&nbsp;&nbsp;&nbsp;&nbsp;
							[前末]&nbsp;総授業数<%= Request("hidSouJyugyou_KZ") %>
									純授業数<%= Request("hidJunJyugyou_KZ") %>
							<BR>
							[後中]&nbsp;総授業数<%= Request("hidSouJyugyou_TK") %>
									純授業数<%= Request("hidJunJyugyou_TK") %>
								&nbsp;&nbsp;&nbsp;&nbsp;
							[学末]&nbsp;総授業数<%= Request("hidSouJyugyou_KK") %>
									純授業数<%= Request("hidJunJyugyou_KK") %>
						</th>
					...
					<tr>
						...
						<%if m_sGakkoNO = cstr(C_NCT_NUMAZU) THEN %>
							<% if m_bKekkaNyuryokuFlg then %>
								<th class="header3" rowspan="2" width="50"  nowrap onClick="f_Paste('Kokyu')"><FONT COLOR="#99CCFF">公休</FONT></th>
							<% else %>
								<th class="header3" rowspan="2" width="50"  nowrap>公休</th>
							<% end if %>
						<% end if %>
