...
'-------------------------------------------------------------------------
' 作      成: 
' 変      更: 2003.02.27:takada:データ更新日の年がm_iNENDOになっていた。
' 変      更: 2003.04.11:hirota:認定状況を学年別でみるように変更
' 変      更: 2005.04.26:西村:沼津用公休欄を追加
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Dim m_sClassMei		'2019/06/12 Add Fujibayashi
	Dim m_sKamokuMei	'2019/06/12 Add Fujibayashi
...
'		if not gf_GetNintei(m_iNendo,m_bNiteiFlg) then
		...
		'//成績登録
		If f_Update(m_sSikenKBN,0) <> 0 Then
			m_bErrFlg = True
			Exit Do
		End If
		...
		If cint(m_sSikenKBN) = cint(C_SIKEN_ZEN_KIM) Then
			If m_bZenkiOnly = True Then
				...
				Else
					'//成績登録(前期のみの試験科目の場合)
					If f_Update(C_SIKEN_KOU_KIM,1) <> 0 Then
						m_bErrFlg = True
						Exit Do
					End If
				End If
...
'*  [説明]  
'********************************************************************************
Function f_Update(p_sSikenKBN, p_lCopyFlg)
	Dim i
	Dim w_Today
	Dim w_FieldName
	Dim w_DataKbnFlg
...
'response.write "(" & cint(gf_SetNull2Zero(request("hidMihyoka"))) & ")<br>"				
				'//未評価、評価不能の設定
				'if cint(gf_SetNull2Zero(request("hidMihyoka"))) <> cint(0) then
				if cint(gf_SetNull2Zero(request("hidMihyoka"))) = C_MIHYOKA then
					w_DataKbn = cint(gf_SetNull2Zero(request("hidMihyoka")))
					w_DataKbnFlg = true
'response.write w_DataKbn & "AAAAAAAAAAAAAA1"
				else
					w_DataKbn = cint(gf_SetNull2Zero(request("chkHyokaFuno" & i)))
'response.write w_DataKbn & "AAAAAAAAAAAAAA2"
			...
			Select Case p_sSikenKBN
				...
				Case C_SIKEN_ZEN_TYU
					...
					if not request("hidNoChange" & i ) Then
						...
						If m_sGakkoNO = C_NCT_NUMAZU Then
							w_sSQL = w_sSQL & " T16_KEKA_NASI_TYUKAN_Z		= " & f_CnvNumNull(request("Kokyu"&i)) & ", "
						END IF
				...
				Case C_SIKEN_ZEN_KIM
					...
					if not request("hidNoChange" & i ) Then
						...
						'//沼津高専の場合 INS2005/04/25 西村
						If m_sGakkoNO = C_NCT_NUMAZU Then
							w_sSQL = w_sSQL & " T16_KEKA_NASI_KIMATU_Z		= " & f_CnvNumNull(request("Kokyu"&i)) & ", "
						END IF
				...
				Case C_SIKEN_KOU_TYU
					...
					if not request("hidNoChange" & i ) Then
						...
						'//沼津高専の場合 INS2005/04/25 西村
						If m_sGakkoNO = C_NCT_NUMAZU Then
							w_sSQL = w_sSQL & " T16_KEKA_NASI_TYUKAN_K		= " & f_CnvNumNull(request("Kokyu"&i)) & ", "
						END IF
				...
				'2003.8.25 ITO
				'前期末試験の成績を学年末にコピーする場合、免除フラグ＝０のデータだけコピーする。 
				'後期末試験の時は、全て更新。
				'(免除フラグ＝0 And 前期末試験の成績を学年末にコピーする場合) Or 後期末試験の時
				'久留米以外の学校は(request("hidMenjyo" & i )に、全て０を設定しているので全科目対象
				If (request("hidMenjyo" & i ) = 0 And p_lCopyFlg = 1) Or p_lCopyFlg = 0 Then
						...
						if not request("hidNoChange" & i ) Then
							...
							'//沼津高専の場合 INS2005/04/25 西村
							If m_sGakkoNO = C_NCT_NUMAZU Then
								w_sSQL = w_sSQL & " T16_KEKA_NASI_KIMATU_K		= " & f_CnvNumNull(request("Kokyu"&i)) & ", "
							END IF
...
%>
	...
	<input type="hidden" name="txtClassMei"   value="<%=trim(Request("txtClassMei"))%>">		<%	'2019/06/12 Add Fujibayashi	%>
	<input type="hidden" name="txtKamokuMei"   value="<%=trim(Request("txtKamokuMei"))%>">		<%	'2019/06/12 Add Fujibayashi	%>
