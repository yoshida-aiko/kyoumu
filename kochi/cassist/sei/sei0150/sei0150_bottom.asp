...
' 変      更: 2007/11/29 結城　　沼津高専　授業時間表示内容を変更
' 変      更: 2009/10/17 岩田　　前期開設科目取得時の条件式の変更（型を変換しそろえる）
' 変      更: 2010/05/20 岩田　 高知高専　混合クラス、一般、指定科目を学科表示にする
' 変      更: 2016/06/14 清本　 高知高専　専門科目をクラス表示にする
' 変      更: 2017/06/29 西村　 高知高専　席次免除フラグ=1の場合は成績入力不可にする(表示のみ)
' 変      更: 2017/09/22 西村　 高知高専　席次免除フラグ参照時の分岐を追加
' 変      更: 2017/12/12 西村　高知高専　混合クラス、一般、指定科目を学科表示にするで140050生物は除外する
' 変      更: 2019/06/12 藤林　高知高専　留年生などクラスのメイン学科ではない生徒への対応(クラス名に学科名を表示する)
' 変      更: 2019/06/13 清本　高知高専　席次免除フラグ=1の場合は遅刻欠席を不可にする(表示のみ)
' 変      更: 2019/06/17 藤林　高知高専　特別活動は、遅刻欠席を入力したいので、席次除外フラグ=0とする
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
  'エラー系
    ...
    Dim m_sClassMei       '//クラス名(留年生などクラスのメイン学科ではない生徒の履修科目の場合は、学科名)	2019/06/12 Add Fujibayashi
    Dim m_sKamokuMei      '//科目名	2019/06/12 Add Fujibayashi
...
'UPDATE 2007/11/29
	IF (m_sGakkoNO = cstr(C_NCT_FUKUSHIMA)) OR (m_sGakkoNO = cstr(C_NCT_NUMAZU)) THEN
...
Sub s_SetParam()
  ...
  m_sClassMei = request("txtClassMei")			'2019/06/12 Add Fujibayashi
  m_sKamokuMei = request("txtKamokuMei")		'2019/06/12 Add Fujibayashi
...
Function f_SikenInfo_T16(p_bZenkiOnly)
  ...
  w_sSQL = w_sSQL & " AND T16_HAITOGAKUNEN = " & m_sGakuNo		'//2019.11.14 Add Kiyomoto
...
'2009.10.17 upd 型変化しないと、条件式が正しく判定されない。
...
'Ins 2017/09/22 nishimura 条件追加
if m_iKamokuKbn = C_JIK_JUGYO then  '通常授業の場合
'席次除外フラグ INS 2017.06.30
  w_sSQL = w_sSQL & "   T16_SEKIJI_JYOGAI_FLG, "
'Ins 2019/06/17 Fujibayashi
else
  '特別活動の場合は、遅刻欠席を入力したいので、除外フラグ=0とする
  w_sSQL = w_sSQL & "   0 AS T16_SEKIJI_JYOGAI_FLG, "
'Ins 2019/06/17 Fujibayashi End
end if
  ...
  if m_iKongou <> C_CLASS_KONGO Then
    ...
    if m_sGakkoNO = C_NCT_KUMAMOTO Then
      ...
      if Cint(m_iKamoku_Kbn) <> C_KAMOKU_SENMON Then
				...
				'UPP Nishimura  OR (m_sKamokuCd = 140050 ) を削除
				If m_sGakkoNO = C_NCT_KOCHI Then
					If (m_sKamokuCd = 140046 ) OR (m_sKamokuCd = 140047 ) OR (m_sKamokuCd = 140048 ) OR (m_sKamokuCd = 180011 ) OR (m_sKamokuCd = 180012 ) OR (m_sKamokuCd = 180013 ) OR (m_sKamokuCd = 180014 ) Then
						w_sSQL = w_sSQL & " AND T13_GAKKA_CD = '" & m_sGakkaCd & "' "
						w_sSQL = w_sSQL & " ORDER BY T13_SYUSEKI_NO1 "
					Else
						w_sSQL = w_sSQL & " AND T13_GAKKA_CD = " & m_sGakkaCd		'2019/06/12 Add Fujibayashi
						w_sSQL = w_sSQL & " AND T13_CLASS = " & cint(m_sClassNo)
		...
		elseIf (m_sGakkoNO = C_NCT_KOCHI) Then
				w_sSQL = w_sSQL & " AND T13_GAKKA_CD = " & m_sGakkaCd				'2019/06/12 Add Fujibayashi
				w_sSQL = w_sSQL & " AND T13_CLASS = " & cint(m_sClassNo)
				w_sSQL = w_sSQL & " ORDER BY T13_SYUSEKI_NO2 "
		'INS 2016/06/14 kiyomoto <--
...
'UPDATE 2007/11/29
  if m_iKamokuKbn = C_JIK_JUGYO then
	  If (m_sGakkoNO = cstr(C_NCT_NIIHAMA)) OR (m_sGakkoNO = cstr(C_NCT_NUMAZU)) then
		  w_sSQL = w_sSQL & " AND NVL(" & w_Table & "_MENJYO_FLG,0) <> 1 "
	  END IF
  end if
...
Sub showPage()
  ...
  Dim w_sInputClass1
  Dim w_sInputClass2
  Dim w_sInputClass3	'Ins 2017/07/05 Nishimura 除外科目用
  ...
  w_sInputClass3 = ""	'//Ins 2017/07/05 Nishimura
  ...
  <%
    ...
    Do Until m_Rs.EOF
      ...
      if not m_bSeiInpFlg Then
        ...
        w_Disabled = "disabled"
      End if
      w_sInputClass3 = "class='" & w_cell & "' style='text-align:right;' readonly tabindex='-1'"
	...
	if (m_sGakkoNO <> C_NCT_NUMAZU) and (m_sGakkoNO <> C_NCT_NIIHAMA) Then
      ...
      <tr>
		...
		<% if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then %>
			...
			<!-- 沼津以外  -->
			<% Else %>
				<!-- 除外フラグが１の場合 Ins2017.06.30 Nishimura-->
          		<% If cint(gf_SetNull2Zero(m_Rs("T16_SEKIJI_JYOGAI_FLG"))) = "1" Then %>

					<td class="<%=w_cell%>" align="center" width="50" nowrap <%=w_Padding%>>

						<input type="text" <%=w_sInputClass3%>  name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size=3 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>);" onChange="f_GetTotalAvg();" style="border-color:#FFFFFF #FFFFFF #FFFFFF #FFFFFF; border-style: solid; text-align:left; vertical-align:middle; " readonly ></td>
				...
				<% Else %>
						...
						<td class="<%=w_cell%>" align="center" width="50" nowrap <%=w_Padding%>>
						<input type="text" <%=w_sInputClass1%>  name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size=3 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>);" onChange="f_GetTotalAvg();"></td>
		...
		<% end if %>
          ...
          <!-- 除外フラグが１の場合に、遅刻欠課を入力不可とする 2019.06.13 Add Kiyomoto -->
          <% If cint(gf_SetNull2Zero(m_Rs("T16_SEKIJI_JYOGAI_FLG"))) = "1" Then %>
              <td class="<%=w_cell%>" align="center" width="55" nowrap <%=w_Padding%>><%=w_sChikai%></td>
              <td class="<%=w_cell%>" align="right" width="55" nowrap <%=w_Padding%>><%=gf_HTMLTableSTR(w_sChikaisu)%></td>
              <td class="<%=w_cell%>" align="center" width="55" nowrap <%=w_Padding%>><%=w_sKekka%></td>
              <td class="<%=w_cell%>" align="right" width="55" nowrap <%=w_Padding%>><%=gf_HTMLTableSTR(w_sKekkasu)%></td>
          <% else %>
			<!-- 遅刻 -->
	          <td class="<%=w_cell%>" align="center" width="55" nowrap <%=w_Padding%>><input type="text" <%=w_sInputClass2%>  name=Chikai<%=i%> value="<%=w_sChikai%>" size=2 maxlength=2 onKeyDown="f_MoveCur('Chikai',this.form,<%=i%>)" onChange="f_CalcSum('Chikai');"></td>
			<!-- 欠課 -->
	          <td class="<%=w_cell%>" align="right"  width="55" nowrap <%=w_Padding%>><%=gf_HTMLTableSTR(w_sChikaisu)%></td>
...
<%'2003.8.24 学校番号をsei0150_upd.aspに渡す為に追加。 ITO%>
  ...
  <input type="hidden" name="txtClassMei"  value="<%=m_sClassMei%>">	<!--'2019/06/12 Add Fujibayashi-->
  <input type="hidden" name="txtKamokuMei"  value="<%=m_sKamokuMei%>">	<!--'2019/06/12 Add Fujibayashi-->
