...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Public m_sHyojiKBNName	'表示項目名
	Public m_sTaniJikanName '「単位数」「授業時間数」の項目名
	
	'配列用
	Dim	   m_iKamokuCd()	'm_Rsの科目コードの配列
	Dim	   m_sKamokuNm()	'm_Rsの科目名の配列
	Dim	   m_iHTaniJikan()		'm_Rsの配当単位または授業時間の配列
...
'********************************************************************************
	...
	Select case m_sHyojiKBN
		case C_SEI0200_SEISEKI
			m_sHyojiKBNName = "成績一覧"
		case C_SEI0200_KEKKA
			m_sHyojiKBNName = "欠課数一覧"
		case C_SEI0200_CHIKOKU
			m_sHyojiKBNName = "遅刻数一覧"
	End Select
End Sub
...
'********************************************************************************
Dim i
Dim w_sStr		'単位数または、授業時間を取得するためのセレクト文
...
'***
	...
	f_getdate = 1
	  ...
	  Select Case m_sHyojiKBN 
		Case C_SEI0200_SEISEKI				'成績の場合は、配当単位数を取得
					w_sStr = "T16_HAITOTANI"
					m_sTaniJikanName = "単　位　数"

		Case Else 							'その他の場合は、授業時間数を取得
					m_sTaniJikanName = "授業時間数"
			Select Case m_sSikenKBN			'取得する欠課数を試験毎のフィールド名にする
				Case C_SIKEN_ZEN_TYU
					w_sStr = "T16_J_JUNJIKAN_TYUKAN_Z"
				Case C_SIKEN_ZEN_KIM
					w_sStr = "T16_J_JUNJIKAN_KIMATU_Z"
				Case C_SIKEN_KOU_TYU
					w_sStr = "T16_J_JUNJIKAN_TYUKAN_K"
				Case C_SIKEN_KOU_KIM
					w_sStr = "T16_J_JUNJIKAN_KIMATU_K"
			End Select
	...
	Do
		...
		w_sSQL = w_sSQL & vbCrLf & "    A.T16_KAMOKU_CD,A.T16_KAMOKUMEI,A.T16_KAMOKU_KBN,MAX(A.T16_SEQ_NO) AS T16_SEQ_NO "
		w_sSQL = w_sSQL & vbCrLf & "    ,MAX(A." & w_sStr & ") AS TANIJIKAN "
...
'		w_sSQL = w_sSQL & vbCrLf & " AND A.T16_GAKKA_CD = B.T13_GAKKA_CD "
		w_sSQL = w_sSQL & vbCrLf & " GROUP BY A.T16_KAMOKU_CD,A.T16_KAMOKUMEI,A.T16_KAMOKU_KBN"',A." & w_sStr & " "',A.T16_SEQ_NO "
...
'response.write m_Rs("T16_SEQ_NO")&"::<BR>"
	        ...
	        ReDim Preserve m_iHTaniJikan(i)
...
'response.write w_iKaisetu&"->"&C_KAI_NASI&"<BR>"
			...
			'If cint(gf_SetNull2Zero(w_iKaisetu)) <> C_KAI_NASI then 
	            ...
	            m_iHTaniJikan(i) = m_Rs("TANIJIKAN")
...
Dim w_sStrH			'表示用
Dim w_sStrD			'表示用
Dim w_sClassMei		'クラス名
Dim w_sGakkaMei		'学科名
Dim w_sSikenMei		'試験名
Dim w_sKamokuKBNMei	'科目区分名


call gf_GetKubunName(C_SIKEN,m_sSikenKBN,m_iNendo,w_sSikenMei)
call gf_GetKubunName(C_HISSEN,m_sKBN,m_iNendo,w_sKamokuKBNMei)

 If m_sKengen <> C_SEI0200_ACCESS_GAKKA then 
	w_sStrH = w_sSikenMei & "(" & w_sKamokuKBNMei & ")"
	w_sStrD = m_sGakuNo & "年　" & gf_GetClassName(m_iNendo,m_sGakuNo,m_sClassNo)
 Else
	w_sStrH = w_sSikenMei & "(" & w_sKamokuKBNMei & ")"
	w_sStrD = m_sGakuNo & "年　" & gf_GetGakkaNm(m_iNendo,m_sGakkaNo)
 End If
...
%>
	...
	<!--
		parent.opener.parent.main.location.href = "default2.asp"
	//-->
	</SCRIPT>
	</head>
	<body>
	...
	<center>
	<% call gs_title(" 成績一覧 ",m_sHyojiKBNName) %>

	<table border="0"><tr><td width="99%">
	<table width="100%">
	<tr>
	<td width=100% valign=top>
		<table class="hyo" border="1" height="20" align=center>
		</table>
		<table class=hyo border=1 align=center>
	    <tr>
	        <th class="header2" width="180" colspan="2"><%=w_sStrH%></th>
	        <th class="header" colspan="<%=m_rCnt%>" nowrap><%=w_sStrD%></th>
	    </tr>
		<tr>
			<th class=header colspan=2 width="180">科　目　名</th>
	<%	For i = 1 to m_rCnt %>
			<td class=detail width="16" align=center valign=top><%=Left(m_sKamokuNm(i),6)%></td>
	<%	Next%>
		</tr>
		<tr>
	...
	<%	Next %>
		...
		<tr>
			<th class=header colspan=2><%=m_sTaniJikanName%></th>
	<%	For i = 1 to m_rCnt %>
			<td class=detail width="26" align=center valign=top><%=m_iHTaniJikan(i)%></td>
	<%	Next%>
		</tr>
		<tr>
	...
	<%	Next%>
		...
		<tr>
			<th class=header2 width="75"><%=gf_GetGakuNomei(m_iNendo,C_K_KOJIN_1NEN)%></th>
			<th class=header2 width="105">氏　名</th>
		</tr>

	<%	For j = 1 to m_SrCnt 
			Call gs_cellPtn(w_cell)%>
		<tr>
			<td class=<%=w_cell%> width="65"><%=m_iGakuseki(j)%></td>
			<td class=<%=w_cell%> width="100"><%=m_sSimei(j)%></td>
		<%	For i = 1 to m_rCnt%>
...
w_sMsg = "対象データは存在しません。条件を入力しなおして検索してください。"
...
<script language="javascript">
<!--
	parent.opener.parent.main.location.href = "default2.asp?txtmsg=<%=w_sMsg%>"
	parent.window.close();
	parent.opener.focus();
