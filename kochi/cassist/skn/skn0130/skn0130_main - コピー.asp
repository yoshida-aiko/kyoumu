...
'* 変      更: 2019/06/11 藤林 留年生などクラスのメイン学科ではない生徒への対応のため、該当する科目のクラス名の欄に学科を表示する
'*************************************************************************/
%>
...
Function f_GetKamoku()
	...
	Do
		...
		for w_num = 1 to ubound(m_AryGakunen)
			...
			w_sSQL = w_sSQL & " 	,DECODE(SUM(CASE WHEN M05_GAKKA_CD = T15_GAKKA_CD THEN 1 ELSE 0 END) , 0 , MAX(M02_GAKKA_KIGO) || T27_GAKUNEN, MAX(M05_CLASSRYAKU) ) AS CLASS_NAME "	'2019.06.11 Ins Fujibayashi
			w_sSql = w_sSql & "  FROM "
			w_sSql = w_sSql & "  T27_TANTO_KYOKAN ,"
			w_sSql = w_sSql & "  M05_CLASS, "
			w_sSql = w_sSql & "  T15_RISYU "
			w_sSQL = w_sSQL & " 	,M02_GAKKA "		'2019.06.11 Ins Fujibayashi
		...
		w_sSQL = w_sSQL & " 	T27_CLASS AS CLASS "				'//2009.11.25 ins iwata クラス単位で表示するため
		w_sSQL = w_sSQL & " 	,DECODE(SUM(CASE WHEN M05_GAKKA_CD = T16_GAKKA_CD THEN 1 ELSE 0 END) , 0 , MAX(M02_GAKKA_KIGO) || T27_GAKUNEN, MAX(M05_CLASSRYAKU) ) AS CLASS_NAME "	'2019.06.11 Ins Fujibayashi
		w_sSQL = w_sSQL & " FROM"
		w_sSQL = w_sSQL & " 	T27_TANTO_KYOKAN,"
		w_sSQL = w_sSQL & " 	T16_RISYU_KOJIN "
		w_sSQL = w_sSQL & " 	,M05_CLASS "	'2019.06.11 Ins Fujibayashi
		w_sSQL = w_sSQL & " 	,M02_GAKKA "	'2019.06.11 Ins Fujibayashi
		w_sSQL = w_sSQL & " WHERE "
	...
	'2018.06.18 Add <--
		...
		w_sSQL = w_sSQL & " and	T16_NENDO = M02_NENDO "
		w_sSQL = w_sSQL & " and	T16_GAKKA_CD = M02_GAKKA_CD "
		'2019.06.11 Ins End
