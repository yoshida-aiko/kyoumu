...
'* 変      更: 2001/06/26 根本
'* 変      更: 2009/11/25 岩田	クラス単位でリストを表示する
'* 変      更: 2018/05/07 清本 履修科目をコース対応する
'* 変      更: 2018/06/19 清本 個人履修追加科目も、前期は「前期・通年」後期は「後期・通年」とする
'* 変      更: 2019/06/24 藤林 試験実施科目一覧は、クラス、学科、科目ごとに表示するように変更。VIEWを使用するように変更
'* 変      更: 2019/06/27 藤林 バグ修正 メイン教官を取得条件に追加
'*************************************************************************/
%>
...
Function f_GetKamoku()
	...
	Do
		...
		'	w_sSql = w_sSql & "  T27_CLASS AS CLASS	"			'//2009.11.25 ins iwata クラス単位で表示するため
		'	w_sSql = w_sSql & "  FROM "
        ...
        '
		'	'w_sSql = w_sSql & "  AND T15_COURSE_CD IN ('0', CASE WHEN M05_COURSE_CD IS NOT NULL THEN M05_COURSE_CD ELSE T15_COURSE_CD END) "	'2018.05.07 Add Kiyomoto
        ...
        '
	''2018.06.18 Add Kiyomoto 前期は「前期・通年」後期は「後期・通年」とする -->
	...
	''201'8.06.18 Add <--
		...
		w_sSQL = w_sSQL & " ,KAMOKU_CD AS KAMOKU"
		w_sSQL = w_sSQL & " ,MAX(KAMOKUMEI) AS KAMOKUMEI"
		w_sSQL = w_sSQL & " ,CLASS"
		w_sSQL = w_sSQL & " ,DECODE(SUM(MAIN_GAKKA), 0, MAX(M02_GAKKA_KIGO) || GAKUNEN, MAX(M05_CLASSRYAKU)) AS CLASS_NAME"
		w_sSQL = w_sSQL & " ,DECODE(SUM(MAIN_GAKKA), 0, MAX(M02_GAKKARYAKSYO), MAX(M05_CLASSMEI)) AS CLASS_NAME_SEND"
		w_sSQL = w_sSQL & " FROM VWEB_RISYU"
		w_sSQL = w_sSQL & " WHERE NENDO = " & m_iSyoriNen 
		w_sSQL = w_sSQL & " AND (KAISETU IN (" & w_sJiki & " ," & C_KAI_TUNEN & ")"
		w_sSQL = w_sSQL & "     OR SIKEN_KBN = " & cint(m_iSikenKbn)
		w_sSQL = w_sSQL & "     )"
		w_sSQL = w_sSQL & " AND KYOKAN_CD = '" & m_iKyokanCd & "' "
		w_sSQL = w_sSQL & " AND KAMOKU_KBN = 0 "					'--0：通常授業 1：特別科目
		w_sSQL = w_sSQL & " AND SIKEN_KBN IS NULL "					'--T27_TANTO_KYOKANとの結合のみでよいのでNULLを条件とする(SIKEN_KBN⇒T26_SIKEN_JIKANWARI.T26_SIKEN_KBNの値)
		w_sSQL = w_sSQL & " AND MAIN_KYOKAN = " & C_MAIN_FLG_YES	'--メイン教官のみ		'--2019.06.27 Add Fujibayashi
		w_sSQL = w_sSQL & " GROUP BY GAKUNEN"
		w_sSQL = w_sSQL & "         ,CLASS"
...
'*          p_Kamoku  ：科目
'*          p_Class	      ：クラスNO
'*  [戻値]  f_GetClassName2：クラス名
'*  [説明]  2009.11.25 iwata f_GetClassNameより作成
'********************************************************************************
Function f_GetClassName2(p_iNendo,p_iGakuNen,p_Kamoku,p_ClassNo)
	Dim w_iRet
	Dim w_sSQL
	Dim rs
	...
	Do
		...
		w_sSql = w_sSql & vbCrLf & " WHERE "
		w_sSql = w_sSql & vbCrLf & "  T27.T27_NENDO=" & p_iNendo
		w_sSQL = w_sSQL & vbCrLf & "  AND T27.T27_CLASS = " & p_ClassNo
		w_sSql = w_sSql & vbCrLf & "  AND T27.T27_GAKUNEN=" & p_iGakuNen
		w_sSql = w_sSql & vbCrLf & "  AND T27.T27_KYOKAN_CD ='" & m_iKyokanCd & "' "
		w_sSql = w_sSql & vbCrLf & "  AND T27.T27_KAMOKU_CD ='" & p_Kamoku & "' "
		w_sSql = w_sSql & vbCrLf & "  AND T27_MAIN_FLG  = " & C_MAIN_FLG_YES
		...
		w_sSql = w_sSql & vbCrLf & "  AND M05.M05_CLASSNO = T27.T27_CLASS "
		w_sSql = w_sSql & vbCrLf & " GROUP BY  "
		w_sSql = w_sSql & vbCrLf & " M05.M05_CLASSRYAKU, T27.T27_CLASS "
...
'response.write w_sSql
	...
	'//戻り値ｾｯﾄ
	f_GetClassName2 = w_sClassName

	'//ﾚｺｰﾄﾞｾｯﾄCLOSE
	Call gf_closeObject(rs)

End Function
'2009.11.25 ins end iwata

'********************************************************************************
'*  [機能]  試験時間割データより実施時間を取得
'*  [引数]  なし
'*  [戻値]  なし
'*  [説明]  2009.11.25 iwata 引数にp_Classを追加
'********************************************************************************
Function f_GetSikenJikan(p_iNendo,p_iGakuNen,p_Kamoku,p_Class,p_sSikenJikan,p_sSJissi,p_sKyositu)
    Dim w_iRet
    Dim w_sSQL
    Dim rs
    ...
    Do
        ...
        w_sSql = w_sSql & vbCrLf & "  AND T26_CLASS ='" & p_Class & "' "		'//2009.11.25 ins iwata
'response.write w_ssql & "<br>"
'response.end
...
<head>
    ...
    //  [説明]
    //************************************************************
    function f_regist(p_Gakunen,p_Class,p_Kamoku,p_ClassName) {

        document.frm.txtGakunen.value=p_Gakunen;
        document.frm.txtClass.value=p_Class;
        document.frm.txtKamoku.value=p_Kamoku;
        document.frm.txtClassName.value=p_ClassName;			//'2019/06/24 Add Fujibayashi
	    ...
	    <table width="80%">
			...
			<%
				...
				do until m_Rs.eof or w_i > C_PAGE_LINE
					...
					w_sSJissi = 0
					w_sKyosituCd = 0
'					w_lJikan = f_GetSikenJikan(m_iSyoriNen,m_Rs("GAKUNEN"),m_Rs("KAMOKU"),w_sSikenJikan,w_sSJissi,w_sKyosituCd)	'2009.11.25 upd  引数にｸﾗｽを追加
					w_lJikan = f_GetSikenJikan(m_iSyoriNen,m_Rs("GAKUNEN"),m_Rs("KAMOKU"),m_Rs("CLASS"),w_sSikenJikan,w_sSJissi,w_sKyosituCd)				'試験時間取得
					...
					Else
						 w_sKyositu = f_GetKyositu(w_sKyosituCd)	'教室名取得
					end if

					'2009.11.25 upd iwata クラス単位で表示する
					'w_className = f_GetClassName(m_iSyoriNen,m_Rs("GAKUNEN"),m_Rs("KAMOKU"),w_sKyosituCd,w_class)'m_Rs("KYOSITU"),w_class)
					w_class = m_Rs("CLASS")

					'2019/06/24 Upd Fujibayashi
					'w_className = f_GetClassName2(m_iSyoriNen,m_Rs("GAKUNEN"),m_Rs("KAMOKU"),w_class)	'クラス名取得
					w_className = m_Rs("CLASS_NAME")	'クラス名取得
					'2019/06/24 Upd End
					...
					%>
						<input type="button" class="button" name="Change" value=">>" onclick="f_regist(<%=gf_HTMLTableSTR(m_Rs("GAKUNEN"))%>,'<%=w_class%>','<%=gf_HTMLTableSTR(m_Rs("KAMOKU"))%>','<%=gf_HTMLTableSTR(m_Rs("CLASS_NAME_SEND"))%>')">
					<%
						End If
					%>
						</td>
		...
		<% End if %>
			...
			<input type="hidden" name="txtClassName" value="">		<% '2019/06/24 Add Fujibayashi %>
