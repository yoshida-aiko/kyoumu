...
<!-- '異動回数追加 -->
	<% if gf_empItem(C_T13_IDOU_NUM) then %>
		<table class="disp" border="1" width="200">
			<tr>
				<td class="disph" width="100" height="16">異動回数</td>
				<td class="disp"><%= gf_SetNull2Zero(m_IDOU_NUM) %> 回 &nbsp</td>
			</tr>
		</table>
	<% End if %>
	<br>
		<table border="1" class=hyo>
			<tr>
				<% if gf_empItem(C_T13_IDOU_KBN) then %>
					<th width="80"  class="header">異動区分</th>
				<% End if %>
			<!--	<% if gf_empItem(C_T13_NENDO) then %>
				...
				<% if gf_empItem(C_T13_IDOU_BI) then %>
					<th width="160" class="header">異動年月日</th>
				<% End if %>
				<% if gf_empItem(C_T13_IDOU_BIK) then %>
					<th width="140" class="header">異動備考</th>
				<% End if %>
			</tr>
			<%
				'// 移動回数分,回す
