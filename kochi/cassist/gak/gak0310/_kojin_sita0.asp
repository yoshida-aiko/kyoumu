...
Function f_GetDetailKihon()
	...
	Do
		...
		w_sSql = w_sSql & " 	B.T11_KIN_TEL,  "		'緊急連絡先
...
Sub showPage()
	...
	Call gf_ComvZip(m_Rs("T11_HON_ZIP"),m_Ken,m_SITYOSONCD,m_SITYOSONMEI,m_TYOIKIMEI,Session("HyoujiNendo"))
	Call gf_ComvZip(m_Rs("T11_GEN_ZIP"),m_Ken2,m_SITYOSONCD2,m_SITYOSONMEI2,m_TYOIKIMEI2,Session("HyoujiNendo"))
			...
			<td valign="top" rowspan="2">
				...
				<table border="1" class="disp">
					<tr><td class="disp">
						...
						if w_bRet = True then
							%><IMG SRC="DispBinary.asp?gakuNo=<%= Session("GAKUSEI_NO") %>" width="88" height="136" border="0"><%
						Else
...
<% '- 住所 - %>
				...
				<table class="disp" border="1" width="260">
					...
					<% End if %>
					<% if gf_empItem(C_T11_KIN_TEL) then %>
						<tr>
							<td class="disph" height="16">緊急連絡先</td>
							<td class="disp"><%= m_KIN_TEL %>&nbsp</td>
						</tr>
					<% End if %>
				</table>
