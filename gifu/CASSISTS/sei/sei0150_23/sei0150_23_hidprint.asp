...
'-------------------------------------------------------------------------
' 作      成: 2005/10/04
' 修      正: 2017/08/04 西村 学生の並び順SQLを修正 2Y01 2Y03 2Y05 のように2Y02が順番に出力されない。2Y02は途中で出力される。
' 修      正: 2018/09/05 西村 和暦元号を関数より取得するように変更(新元号対応)
' 修　    正: 2019/07/31 清本 成績出力に、必修か選択科目のうち選択している学生のみを取得する様に修正
' 修　    正: 2020/01/15 Chromeで印刷できるように修正
'*************************************************************************/
%>
...
'INS 20070221
	w_sSQL = w_sSQL & ",DECODE(" & w_Table & "_GAKKA_CD, " & w_Table & "_CURRI_GAKKA_CD,0,1) AS SORTS "
...
'	w_sSQL = w_sSQL & " 	AND	T13_CLASS   = " & cint(m_sClassNo)
	...
	if m_iKamokuKbn = C_JIK_JUGYO then
		...
		'//必修か選択科目のうち選択している学生のみを取得する		'INS 2019/07/31 清本
...
'INS 20070221
	w_sSQL = w_sSQL & " AND ((" & w_Table & "_GAKKA_CD = " & w_Table & "_CURRI_GAKKA_CD) OR (" & w_Table & "_GAKKA_CD <> " & w_Table & "_CURRI_GAKKA_CD"
	w_sSQL = w_sSQL & " AND (" & w_Table & "_HISSEN_KBN = 1 OR (" & w_Table & "_HISSEN_KBN = 2 AND " & w_Table & "_SELECT_FLG = 1 ))))"
...
'DEL 20170804
'INS 20070221
'	w_sSQL = w_sSQL & " ORDER BY SORTS, " & w_Table & "_GAKUSEKI_NO "


'INS 20170804西村 並び順不具合のため 
	w_sSQL = w_sSQL & " ORDER BY SORTS, " & "GAKUSEKI_NO "
...
<form name="frm" method="post">
	...
	<table aling="center">
		<tr>
			...
			<td class="head3" aling="center"><%=f_Nendo2Wareki(m_iNendo)%>年度</td>
		...
		<tr>
			<td>
				<table class="hyo2" align="center" border="1" cellspacing=0 cellpadding=1 width="100%" >
					...
					<tr height="30">
						<td class="head1" width="5%" align="center" nowrap></td>
