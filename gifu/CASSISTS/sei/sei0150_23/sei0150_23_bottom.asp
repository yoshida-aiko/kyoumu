...
' 修　    正: 2005/11/09 西村 休学の日付の判定を異動修了日と判定するように修正
' 修　    正: 2017/08/04 西村 学生の並び順SQLを修正 2Y01 2Y03 2Y05 のように2Y02が順番に出力されない。2Y02は途中で出力される。
' 修　    正: 2019/02/26 藤林 成績登録時に、必修か選択科目のうち選択している学生のみを取得する様に修正
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
    ...
    Dim m_iIdouEnd        '//異動対象期間終了日 NS 2005/11/09 西村
...
Function f_Nyuryokudate()
	...
	w_sSQL = ""
	w_sSQL = w_sSQL & " SELECT "
	w_sSQL = w_sSQL & " 	T24_SEISEKI_KAISI, T24_IDOU_SYURYO,"
	...
	Else
		...
		m_iIdouEnd = gf_SetNull2String(w_Rs("T24_IDOU_SYURYO"))  '異動対象終了 INS 2005/11/09 西村
	End If
...
Sub s_IdouCheck(p_GakusekiNo,p_IdouKbn,p_IdouName,p_bNoChange)
	...
	Dim w_IdouDate		'INS 2005/11/09 西村
	...
	m_Date = m_iIdouEnd  'INS 2005/11/09 西村

	'INS 2005/11/09 西村
	Call f_Get_IdouChk(p_GakusekiNo,m_Date,m_iNendo,w_IdoutypeName,p_IdouKbn,w_IdouDate)

	'p_IdouKbn = gf_Get_IdouChk(p_GakusekiNo,m_Date,m_iNendo,w_IdoutypeName) DEL 2005/11/09 西村 表定評出力と同じ処理にする
...
'*			p_Date		:授業実施日
'*	[戻値]	0:情報取得成功 99:失敗
'*	[説明]	2001.12.19 版：岡田  INS 2005/11/09 西村 HidPrintよりコピー
'********************************************************************************
