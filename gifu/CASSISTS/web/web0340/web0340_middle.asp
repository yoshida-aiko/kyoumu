...
Dim w_iNyuNendo
    ...
    Do
        ...
        m_sSQL = m_sSQL & vbCrLf & "     T15_KAMOKUMEI,T15_KAMOKU_CD,T15_HAITO"&Cint(m_sGakunen)&" "
        m_sSQL = m_sSQL & vbCrLf & " FROM "
        m_sSQL = m_sSQL & vbCrLf & "     T15_RISYU "
        m_sSQL = m_sSQL & vbCrLf & " WHERE "
        m_sSQL = m_sSQL & vbCrLf & "     T15_NYUNENDO = " & w_iNyuNendo & " "
        m_sSQL = m_sSQL & vbCrLf & " AND T15_HISSEN_KBN = " & C_HISSEN_SEN & " "
        m_sSQL = m_sSQL & vbCrLf & " AND T15_HAITO"&Cint(m_sGakunen)&" <> " & C_T15_HAITO & " "
        m_sSQL = m_sSQL & vbCrLf & " AND T15_GRP = " & m_sGRP & " "
        m_sSQL = m_sSQL & vbCrLf & " AND T15_KAMOKU_KBN = " & m_sKBN & " "
        m_sSQL = m_sSQL & vbCrLf & " AND T15_GAKKA_CD = " & m_sClass & " "
...
'******************************************************************
    ...
    Do
        ...
        m_sSQL = m_sSQL & " AND T16_KAMOKU_CD = '" & m_Krs("T15_KAMOKU_CD") & "' "
...
'response.write m_sSQL
        ...
        If m_KSrs.EOF Then
            m_KSrCnt = "0"%>
	        <td class=disph><%=m_Krs("T15_KAMOKUMEI")%></td>
...
<link rel=stylesheet href="../../common/style.css" type=text/css>
  ...
  <% End If %>
	<table class=hyo border=1>
	    <tr>
	        <th class=header width=110 ><%=gf_GetGakuNomei(m_iNendo,C_K_KOJIN_1NEN)%></th>
	    ...
	    <%
	        ...
	        Do Until m_Krs.EOF
		        n = n + 1
			    %>
		        <th class=header2 width=96 valign=middle><%=m_Krs("T15_KAMOKUMEI")%>
		        <input type=hidden name=kamokuCd<%=n%> value="<%=m_Krs("T15_KAMOKU_CD")%>">
		        <input type=hidden name=Tanisuu<%=n%> value="<%=m_Krs("T15_HAITO"&Cint(m_sGakunen)&"")%>"></th>
			    <%
		        m_Krs.MoveNext
	        Loop%>
	    </tr>
