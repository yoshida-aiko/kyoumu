...
' 変      更：2008/11/04 西村 彩和子   岐阜の場合指導上参考となる諸事項は200文字
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
    ...
    Dim  m_SchoolNo             '学校番号 Ins2008/11/04
...
'Response.Write mC_SCHOOL_NO_GIFU
'Ins_end 2008/11/04
...
'        w_sSQL = w_sSQL & "     T11_KODOSYOKEN,T11_SYUMITOKUGI,T11_TYOSA_BIK "
        ...
        w_sSQL = ""
        w_sSQL = w_sSQL & " SELECT "
        w_sSQL = w_sSQL & "     T13_TOKUKATU_DET,T13_NENSYOKEN "
        w_sSQL = w_sSQL & " FROM "
...
//        }
		...
		<% if Cint(m_SchoolNo) = Cint(mC_SCHOOL_NO_GIFU) then %>
	        if( getLengthB(document.frm.NSyoken.value) > "200" ){
	            window.alert("指導上参考となる諸事項の欄は全角100文字以内で入力してください");
...
%>
                <TR>
					...
					<% if Cint(m_SchoolNo) = Cint(mC_SCHOOL_NO_GIFU) then %>
                    <TD CLASS="detail"><textarea rows=4 cols=50 class=text name="NSyoken" onChange="chk_Flg=true;"><%=m_URs("T13_NENSYOKEN")%></textarea><br>
                    <font size=2>
						（全角100文字以内）
					<% ELSE %>
                    <TD CLASS="detail"><textarea rows=2 cols=50 class=text name="NSyoken" onChange="chk_Flg=true;"><%=m_URs("T13_NENSYOKEN")%></textarea><br>
                    <font size=2>
						（全角50文字以内）
					<% END IF %>
					</font></TD>
