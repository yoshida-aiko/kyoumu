...
' 変      更: 2003/09/08 Nishimura T16免除ﾌﾗｸﾞ判定追加
' 変      更: 2004/02/13 shiki 学年末の欠課は前期期末、後期中間の合計を表示、
'							   各試験は、それぞれの欠課数を表示
'*************************************************************************/
%>
...
Function f_GetStudent()
	...
	Select Case m_sSikenKBN
		...
		Case C_SIKEN_ZEN_KIM	'前期期末
			...
			w_sSQL = w_sSQL & w_Table & "_KEKA_KIMATU_Z      AS KEKA,"				'欠課（前期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_KIMATU_Z AS TEISI_ZK,"			'停止（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_KIMATU_Z AS TEISI,"				'停止（前期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_KIMATU_Z  AS HAKEN_ZK,"			'派遣（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_KIMATU_Z  AS HAKEN,"				'派遣（前期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KIBI_KIMATU_Z      AS KIBI_ZK, "			'忌引（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KIBI_KIMATU_Z      AS KIBI, "				'忌引（前期期末）←これを表示
		...
		Case C_SIKEN_KOU_TYU	'後期中間
			...
			w_sSQL = w_sSQL & w_Table & "_KEKA_KIMATU_Z      AS KEKA_ZK,"			'欠課（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KEKA_TYUKAN_K      AS KEKA,"				'欠課（後期中間）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_KIMATU_Z AS TEISI_ZK,"			'停止（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_TYUKAN_K AS TEISI,"				'停止（後期中間）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_KIMATU_Z  AS HAKEN_ZK,"			'派遣（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_TYUKAN_K  AS HAKEN,"				'派遣（後期中間）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KIBI_KIMATU_Z      AS KIBI_ZK, "			'忌引（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KIBI_TYUKAN_K      AS KIBI, "				'忌引（後期中間）←これを表示
		...
		Case C_SIKEN_KOU_KIM	'後期期末
			...
			w_sSQL = w_sSQL & w_Table & "_KEKA_KIMATU_K      AS KEKA,"				'欠課（後期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_TYUKAN_Z AS TEISI_ZT,"			'停止（前期中間）
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_KIMATU_Z AS TEISI_ZK,"			'停止（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_TYUKAN_K AS TEISI_KT,"			'停止（後期中間）
			w_sSQL = w_sSQL & w_Table & "_KEKA_NASI_KIMATU_K AS TEISI,"				'停止（後期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_KIMATU_Z  AS HAKEN_ZK,"			'派遣（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_TYUKAN_K  AS HAKEN_KT,"			'派遣（後期中間）
			w_sSQL = w_sSQL & w_Table & "_KOUKETSU_KIMATU_K  AS HAKEN,"				'派遣（後期期末）←これを表示
			w_sSQL = w_sSQL & w_Table & "_KIBI_KIMATU_Z      AS KIBI_ZK, "			'忌引（前期期末）
			w_sSQL = w_sSQL & w_Table & "_KIBI_TYUKAN_K      AS KIBI_KT, "			'忌引（後期中間）
			w_sSQL = w_sSQL & w_Table & "_KIBI_KIMATU_K      AS KIBI, "				'忌引（後期期末）←これを表示
	...
	if m_iKamokuKbn = C_JIK_JUGYO then
		...
		w_sSQL = w_sSQL & "     T16_MENJYO_FLG AS MENJYO_FLG, "	'INS 2003.09.08 西村　免除ﾌﾗｸﾞ追加
	end if
...
'********************************************************************************
Function f_GetKekaChi(p_iNendo,p_iSikenKBN,p_sKamokuCD,p_sGakusei,p_iKekka,p_iChikoku,p_iKibiki,p_iHaken,p_iTeisi)
	Dim w_sSQL
	Dim w_Rs
	Dim w_sKek,w_sChi,w_sKibi,w_sHaken,w_sTeisi
	...
	w_sSQL = w_sSQL & 	w_sKibi  & " as KIBIKI, "
	w_sSQL = w_sSQL & 	w_sHaken & " as HAKEN "
	...
	If w_Rs.EOF = False Then
		...
		p_iKibiki = gf_SetNull2String(w_Rs("KIBIKI"))
		p_iHaken = gf_SetNull2String(w_Rs("HAKEN"))
		p_iTeisi = gf_SetNull2String(w_Rs("KEKA_NASI"))
	End If
...
Sub s_IdouCheck(p_GakusekiNo,p_IdouKbn,p_IdouName,p_bNoChange)
	...
	w_IdoutypeName = ""
	'DEL 2003.09.08 Nishimura 関数s_SetGrades()でデフォルトセットを行うため
	'p_IdouName = ""
...
'********************************************************************************
'*  [機能] 成績のセット
'*変更	:2003.09.08 Nishimura 引数　p_IdouNameを追加
'********************************************************************************
Sub s_SetGrades(p_sSeiseki,p_sHyoka,p_IdouName,p_bNoChange)

	p_IdouName = ""		'INS 2003.09.08 Nishimura
...
'	End If
	...
	if m_iKamokuKbn = C_JIK_JUGYO then
		...
		'INS 2003.09.08 西村　免除ﾌﾗｸﾞ
		IF gf_SetNull2String(m_Rs("MENJYO_FLG")) = "1" then
			p_bNoChange = True
			p_IdouName = "[免除]"
		End if
	End if
...
'*  [機能] 欠課、遅刻の日々計の取得
'********************************************************************************
Sub s_SetKekkaTotal_BK(p_sKekkasu,p_sChikaisu,p_sKibiki,p_sHaken,p_sTeisi)
	Dim w_sData
	Dim w_iKekka_rui,w_iChikoku_rui,w_iKibik,w_iHaken,w_iTeisi
	...
	if cint(m_iSyubetu) = cint(C_K_KEKKA_RUISEKI_KEI) and m_sSikenKBN <> C_SIKEN_ZEN_TYU then
		...
		'以前の試験で登録されているデータを取得
		call f_GetKekaChi(m_iNendo,m_iShikenInsertType,m_sKamokuCd,m_Rs("GAKUSEI_NO"),w_iKekka_rui,w_iChikoku_rui,w_iKibik,w_iHaken,w_iTeisi)
		...
		'忌引どちらも""の時は""
		if p_sKibiki = "" and w_iKibik = "" then
			p_sKibiki = ""
		else
			p_sKibiki = cint(gf_SetNull2Zero(p_sKibiki)) + cint(gf_SetNull2Zero(w_iKibik))
		end if
		'派遣どちらも""の時は""
		if p_sHaken = "" and w_iHaken = "" then
			p_sHaken = ""
		else
			p_sHaken = cint(gf_SetNull2Zero(p_sHaken)) + cint(gf_SetNull2Zero(w_iHaken))
		end if
		'停止どちらも""の時は""
		if p_sTeisi = "" and w_iTeisi = "" then
			p_sTeisi = ""
		else
			p_sTeisi = cint(gf_SetNull2Zero(p_sTeisi)) + cint(gf_SetNull2Zero(w_iTeisi))
		end if
...
'*  [機能] 欠課、遅刻の日々計の取得
'********************************************************************************
Sub s_SetKekkaTotal(p_sKekkasu,p_sChikaisu,p_sKibiki,p_sHaken,p_sTeisi)
	Dim w_sData
	Dim w_iKekka_rui,w_iChikoku_rui,w_iKibik,w_iHaken,w_iTeisi
	Dim w_iKekka_rui2,w_iChikoku_rui2,w_iKibik2,w_iHaken2,w_iTeisi2
	...
	if cint(m_iSyubetu) = cint(C_K_KEKKA_RUISEKI_KEI) and m_sSikenKBN <> C_SIKEN_ZEN_TYU then
			...
			' ■■■ 後期期末の欠課数は、前期期末と後期中間の合計を表示する
			'■ 前期期末データ取得 -----------------------
			call f_GetKekaChi(m_iNendo,C_SIKEN_ZEN_KIM,m_sKamokuCd,m_Rs("GAKUSEI_NO"),w_iKekka_rui,w_iChikoku_rui,w_iKibik,w_iHaken,w_iTeisi)
			'■ 後期中間データ取得 -----------------------
			call f_GetKekaChi(m_iNendo,C_SIKEN_KOU_TYU,m_sKamokuCd,m_Rs("GAKUSEI_NO"),w_iKekka_rui2,w_iChikoku_rui2,w_iKibik2,w_iHaken2,w_iTeisi2)

			'欠課
			if p_sKekkasu = "" and w_iKekka_rui = "" and w_iKekka_rui2 = "" then
				p_sKekkasu = ""
			else
				p_sKekkasu = cint(gf_SetNull2Zero(w_iKekka_rui)) + cint(gf_SetNull2Zero(w_iKekka_rui2))
			end if

			'どちらも""の時は""
			if p_sChikaisu = "" and w_iChikoku_rui = "" and w_iChikoku_rui2 = "" then
				p_sChikaisu = ""
			else
				p_sChikaisu = cint(gf_SetNull2Zero(w_iChikoku_rui)) + cint(gf_SetNull2Zero(w_iChikoku_rui2))
			end if

			'忌引どちらも""の時は""
			if p_sKibiki = "" and w_iKibik = "" and w_iKibik2 = "" then
				p_sKibiki = ""
			else
				p_sKibiki = cint(gf_SetNull2Zero(w_iKibik)) + cint(gf_SetNull2Zero(w_iKibik2))
			end if

			'派遣どちらも""の時は""
			if p_sHaken = "" and w_iHaken = "" and w_iHaken2 = "" then
				p_sHaken = ""
			else
				p_sHaken = cint(gf_SetNull2Zero(w_iHaken)) + cint(gf_SetNull2Zero(w_iHaken2))
			end if

			'停止どちらも""の時は""
			if p_sTeisi = "" and w_iTeisi = "" and w_iTeisi2 = "" then
				p_sTeisi = ""
			else
				p_sTeisi = cint(gf_SetNull2Zero(w_iTeisi)) + cint(gf_SetNull2Zero(w_iTeisi2))
			end if
...
<!--
	...
	<%
		...
		Do Until m_Rs.EOF
			...
			'//成績データセット
			Call s_SetGrades(w_sSeiseki,w_sHyoka,w_IdouName,w_bNoChange)
			...
			'20040212 del shiki ------------------------------------------------
			'//欠課、遅刻の日々計の取得
			'Call s_SetKekkaTotal(w_sKekkasu,w_sChikaisu,w_sKibi,w_sHaken,w_sTeisi)

			'20040212 add shiki ------------------------------------------------
			' ■■■ 後期期末の欠課数は、前期期末と後期中間の合計を表示する
			'//欠課、遅刻の日々計の取得
			If m_sSikenKBN = C_SIKEN_KOU_KIM then
				Call s_SetKekkaTotal(w_sKekka,w_sChikaisu,w_sKibi,w_sHaken,w_sTeisi)
			End If
