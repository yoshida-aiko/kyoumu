...
'-------------------------------------------------------------------------
' 作      成: 廣田
' 変      更: 2009/10/02 iwata  後期開設科目の成績を学年末に設定する
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Const C_K_TAIHI_MI = "0"		'後期中間データ退避未処理 2009.10.02 ins
	Const C_K_TAIHI_SUMI = "1"	'後期中間データ退避処理済 2009.10.02 ins
	...
	Dim m_iTaihi				'後期中間データ退避区分(C_K_TAIHI_MI(0):未、C_K_TAIHI_SUMI(1):済) 2009.10.02 ins
	Dim m_SchoolFlg
	Dim m_bSeiInpFlg
	Dim m_HyokaDispFlg
...
'2009.10.02 ins str iwata
		'//後期中間データ退避区分取得
		if not gf_GetTaihiInfo(m_iNendo,m_iTaihi) then
...
Sub s_SetParam()
	...
	m_bKokiOnly      = cbool(request("hidKokiOnly"))		'//2009.10.02 ins
...
Function f_Update(p_sSikenKBN)
	...
	Do
		...
		For i=1 to i_max
			...
			'//INS 2004/12/17 Amano 免除フラグの有無を確認
			w_Menjo = CInt(gf_SetNull2Zero(Request("chkMenjo_Flg" & i)))

			'//INS 2004/12/17 Amano 免除フラグが立っていない場合はUPDATE処理を行う
			If w_Menjo = 0 Then
				'//評価不能チェック(熊本電波のみ)
				...
				Select Case p_sSikenKBN
					...
					Case C_SIKEN_KOU_TYU
						If Not request("hidNoChange" & i ) Then
							...
							'2009.10.02 ins str iwata
							'後期中間データ退避処理済で、後期開設科目のならば、学年末成績にも登録する、
							if (cstr(m_iTaihi) = C_K_TAIHI_SUMI and m_bKokiOnly ) then
								'//数値入力
								If m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then
...
'********************************************************************************
'*  [機能]  管理マスタより後期中間データ退避処(51)を取得
'*  [引数]  なし
'*  [戻値]  p_sSyubetu = C_K_TAIHI_MI : 未(=0)
'*  [戻値]  p_sSyubetu = C_K_TAIHI_SUMI   ：済(=1)
'*  [説明]
'********************************************************************************
Function gf_GetTaihiInfo(p_iNendo,p_iSyubetu)
    Dim w_sSQL
    Dim w_Rs
    Dim w_iRet
    ...
    gf_GetTaihiInfo = false
	p_iSyubetu=0

    Do

		'//管理マスタより後期中間データ退避区分を取得
		w_sSQL = ""
		...
		w_sSQL = w_sSQL & vbCrLf & "  AND M00_KANRI.M00_NO=51"	'後期中間データ退避区分(=51)
