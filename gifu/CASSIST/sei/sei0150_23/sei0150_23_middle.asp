		...
		<tr>
			...
			<form name="frm" method="post">
				<table border=1 class=hyo width=670>
					...
					<tr>
						<th class="header3 border-white" width="96"  align="center">成績入力期間</th><td class=detail width="239"  align="center" colspan="2"><%=m_FromSei%> ～ <%=m_ToSei%></td>
					...
					<tr>
						<th class="header3 border-white" width="96"  align="center">実施科目</th>
						<%
		...
		<tr>
			<td align="center">
				...
				<% if m_bSeiInpFlg or m_bKekkaNyuryokuFlg Then %>
					...
					<% else %>
						<input type="button" class="button" value="　登　録　" name="btnINP" onclick="f_Touroku();">
					<% end if %>
				<% end if %>
				...
				<table class="hyo" border="1" align="center" width="<%=m_TableWidth%>" style="border-color:#FFF">
					<tr>
						...
						<% if m_sSikenKBN = C_SIKEN_KOU_KIM then %>
							...
							-->
								&nbsp;&nbsp;授業時間数計：&nbsp;<%= Request("hidJunJyugyou_KK") %>
							</th>
						<% end if %>
					</tr>
					<tr>
						<th class="header3 padding1" rowspan="2" width="65" nowrap><%=gf_GetGakuNomei(m_iNendo,C_K_KOJIN_1NEN)%></th>
						<th class="header3 padding1" rowspan="2" width="150" nowrap>氏　名</th>
						<th class="header3 padding1" colspan="4" nowrap>成績履歴</th>

						<% if m_iSeisekiInpType = cint(C_SEISEKI_INP_TYPE_NUM) and m_bSeiInpFlg then %>
							<th class="header3 padding1" rowspan="2" width="50" nowrap onClick="f_Paste('Seiseki')"><FONT COLOR="#99CCFF">成績</FONT></th>
						<% else %>
							<th class="header3 padding1" rowspan="2" width="50" nowrap>成績</th>
						<% end if %>

						<% If cstr(m_iKamokuKbn) = cstr(C_JIK_JUGYO) then %>
							<th class="header3 padding1" colspan="4" nowrap>欠課</th>
						<% end if %>
					</tr>

					<tr>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;">前中</span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;">前末</span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;">後中</span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;">学末</span></th>


						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;" onClick="f_Paste('txtKekka')"><FONT COLOR="#99CCFF">欠課</FONT></span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;" onClick="f_Paste('txtKibi')"><FONT COLOR="#99CCFF">忌引</FONT></span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;" onClick="f_Paste('txtTeisi')"><FONT COLOR="#99CCFF">停止</FONT></span></th>
						<th class="header2 padding1" width="50" nowrap><span style="font-size:10px;" onClick="f_Paste('txtHaken')"><FONT COLOR="#99CCFF">派遣</FONT></span></th>

					</tr>
				</table>
			</td>
			<td width='<%=Cint(request("scrollbarWidth"))%>' style="margin:0px;padding:0px;"></td><%' 2022.03.24 Ins スクロールバーの幅の分調整 %>	
		</tr>
	</table>
