...
' 変      更: 2015/12/25 fujibayashi  学年成績入力チェック追加(前中/前末/後中で登録した範囲内の数値しか入力できない)
' 変      更: 2018/03/12 Nishimura 入力チェックに0不可を追加
' 変      更: 2018/03/12 Nishimura 学年末成績入力時、成績最大最少チェック判定の授業時間数を（忌引き-派遣-出席停止)を差し引いて判定するように修正
'*************************************************************************/
%>
...
'********************************************************************************
'*  [機能]  検索対象の配列値で、最大値もしくは最小値を取得する
'*  [引数]  p_iNumber()　：　検索対象
'*          p_sType	   　：　MAX or MIN
'*          p_bZero	   　：　True:0を対象とする　False:0を対象としない
'*  [戻値]  数値：最大値もしくは、最小値
'*  [説明]  
'*			2015.12.25 
'*			
'********************************************************************************
Function f_GetMinMax(p_iNumber(), p_sType, p_bZero)
    ...
    '//最大値もしくは最小値を取得する 学年末は含めないので-1する
    For intCnt = 0 To UBound(p_iNumber) - 1 
        Select Case p_sType
        Case "MIN"
            If (p_bZero = True Or p_iNumber(intCnt) > 0) And p_iNumber(intCnt) < w_iNumber Then
                w_iNumber = p_iNumber(intCnt)
            End If
        Case Else
            If p_iNumber(intCnt) > w_iNumber Then
                w_iNumber = p_iNumber(intCnt)
            End If
        End Select
    Next

    f_GetMinMax = w_iNumber
...
<!--
	...
	function f_CheckNum(pFromName){
		...
		if(isNaN(wFromName.value)){
			...
			if(wFromName.name.indexOf("Seiseki") != -1){
				//Ins_s 2018.03.12 Nishimura 0 は入力不可
				if(wFromName.value == 0){
					wFromName.focus();
					wFromName.select();
					return false;
				}
				//Ins_e 2018.03.12 Nishimura 0 は入力不可
				...
				//学年末試験の場合のみ
				if(eval("document.frm.sltShikenKbn").value == 4){
					//授業時間数の1/4を超える場合はチェックをしない ins2016.02.22 Nishimura
					wJunjikan = parent.topFrame.document.frm.txtJunJyugyou.value;
					if(wJunjikan == 0){	
					...
					for (w_li = 1; w_li <= 99; w_li++) {
						...
						if (wFromName.name == "Seiseki" + w_li){
							wSEIMin = eval("document.frm.hidMin" + w_li).value;
							wSEIMax = eval("document.frm.hidMax" + w_li).value;
							wKekka  = eval("document.frm.hidKeka" + w_li).value;
							//Ins_s  2018.03.12 Nishimura (忌引き-派遣-出席停止)を授業時間から差し引く 欠課一覧表の判定と同様に
							wKibi  = eval("document.frm.hidKibi" + w_li).value;
							wHaken  = eval("document.frm.hidHaken" + w_li).value;
							wTeisi  = eval("document.frm.hidTeisi" + w_li).value;
							wJunjikan2 = wJunjikan -  (wKibi + wHaken + wTeisi);
							//Ins_e 2018.03.12 Nishimura

							if(wKekka < wJunjikan2 / 4 ){
							//if(wKekka < wJunjikan/4 ){
								
								if(parseInt(wFromName.value) < parseInt(wSEIMin) || parseInt(wFromName.value) > parseInt(wSEIMax)){
									wFromName.focus();
								wFromName.select();
									return false;
								}else{
	...
	<%
		...
		Do Until m_Rs.EOF
			...
			if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then
				redim wSEISEKI(3)
				wSEISEKI(0) = cint(gf_SetNull2Zero(m_Rs("SEI1")))
				wSEISEKI(1) = cint(gf_SetNull2Zero(m_Rs("SEI2")))
				wSEISEKI(2) = cint(gf_SetNull2Zero(m_Rs("SEI3")))
				wSEISEKI(3) = cint(gf_SetNull2Zero(m_Rs("SEI4")))

				wSEIMin = f_GetMinMax(wSEISEKI,"MIN",false)
				wSEIMax = f_GetMinMax(wSEISEKI,"MAX",false)
			end if
			...
			<tr>
				...
				<input type="hidden" name="hidMin<%=i%>" value="<%=wSEIMin%>">
				<input type="hidden" name="hidMax<%=i%>" value="<%=wSEIMax%>">
				...
				<% Else %>
					...
					<% Else %>
						...
						<% if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then %>
							<td class="<%=w_cell%>" align="right" width="50" nowrap <%=w_Padding2%>>
								<input type="text" style="width:48px" <%=w_sInputClass1%> name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size="2" maxlength="3" onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>);" onChange="f_GetTotalAvg();">
					...
					<td class="<%=w_cell%>" align="center" width="50"  nowrap <%=w_Padding2%>>
						<% If w_Menjo = 1 Then %>
							<%=w_sKekka%>
						<% Else %>
							<input type="text" style="width:48px" <%=w_sInputClass2%> name="txtKekka<%=i%>" value="<%=w_sKekka%>" onKeyDown="f_MoveCur('txtKekka',this.form,<%=i%>);" size="3" maxlength="3">
						<% End If %>
						<input type="hidden" name="hidKeka<%=i%>" value="<%=w_sKekka%>"> 
					...
					<td class="<%=w_cell%>" align="center" width="50"  nowrap <%=w_Padding2%>>
						<% If w_Menjo = 1 Then %>
							<%=w_sKibi%>
						<% Else %>
							<input type="text" style="width:48px" <%=w_sInputClass2%> name="txtKibi<%=i%>"  value="<%=w_sKibi%>" onKeyDown="f_MoveCur('txtKibi',this.form,<%=i%>);" size="3" maxlength="2">
						<% End If %>
						<input type="hidden" name="hidKibi<%=i%>" value="<%=w_sKibi%>">
					...
					<td class="<%=w_cell%>" align="center" width="50"  nowrap <%=w_Padding2%>>
						<% If w_Menjo = 1 Then %>
							<%=w_sTeisi%>
						<% Else %>
							<input type="text" style="width:48px" <%=w_sInputClass2%> name="txtTeisi<%=i%>" value="<%=w_sTeisi%>" onKeyDown="f_MoveCur('txtTeisi',this.form,<%=i%>);" size="3" maxlength="3">
						<% End If %>
						<input type="hidden" name="hidTeisi<%=i%>" value="<%=w_sTeisi%>">
					...
					<td class="<%=w_cell%>" align="center" width="50"  nowrap <%=w_Padding2%>>
						<% If w_Menjo = 1 Then %>
							<%=w_sHaken%>
						<% Else %>
							<input type="text" style="width:48px" <%=w_sInputClass2%> name="txtHaken<%=i%>" value="<%=w_sHaken%>" onKeyDown="f_MoveCur('txtHaken',this.form,<%=i%>);" size="3" maxlength="2">
						<% End If %>
						<input type="hidden" name="hidHaken<%=i%>" value="<%=w_sHaken%>">
...
//-->
	...
	<input type="hidden" name="scrollbarWidth" value="<%=scrollbarWidth%>">	<!-- 2022/03/24 追加 -->
