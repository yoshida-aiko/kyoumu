...
'UPDATE 2011/02/25 ORDER BY 変更
'		w_sSql = w_sSql & vbCrLf & " ORDER BY T16_SEQ_NO"
		w_sSql = w_sSql & vbCrLf & " ORDER BY T16_KAMOKU_KBN,T16_HISSEN_KBN,T16_SEQ_NO"
...
'response.end
		...
		Do Until w_Rs.EOF
			...
			do 
				...
				'	Exit Do
				'End If
		        if m_SchoolNo = "11" or m_SchoolNo = "55" or m_SchoolNo = "23" then  '福島高専の場合 岐阜も追加(2011/02/25)
					Select Case cint(gf_SetNull2Zero(m_sSikenKBN))
