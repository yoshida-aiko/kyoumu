...
' システム名: 教務事務システム
' 処  理  名: 連絡掲示板
' ﾌﾟﾛｸﾞﾗﾑID : web/web0330/web0330_DEL.asp
' 機      能: 上ページ 表示情報を表示
'-------------------------------------------------------------------------
...
'******************************************************************
    ...
    Do
        ...
        m_sSQL = m_sSQL & "     T46_NO,T46_KENMEI "
        m_sSQL = m_sSQL & " FROM "
        m_sSQL = m_sSQL & "     T46_RENRAK "
        m_sSQL = m_sSQL & " WHERE "
        If m_stxtMode = "" Then
            m_sSQL = m_sSQL & "     T46_NO IN (" & Trim(m_sNo) & ") "
        ElseIf m_stxtMode = "DELKNIN" Then
            m_sSQL = m_sSQL & "     T46_NO = '" & Trim(m_sDelNo) & "' "
        End If
...
'******************************************************************
    ...
    Do
        '//リストの表示
        m_sSQL = ""
        m_sSQL = m_sSQL & " DELETE FROM T46_RENRAK "
        m_sSQL = m_sSQL & " WHERE "
        m_sSQL = m_sSQL & "     T46_NO IN (" & Trim(m_sDelNo) & ") "
...
<%call gs_title("連絡掲示板","削　除")%>
<br>
削　除　内　容
<br><br>
    <table border="1" class=hyo width="40%">
<form name="frm" action="post">

    <tr>
    <th class=header>処理番号</th>
    <th class=header>件名</th>
    </tr>
...
%>
    <tr>
    <td align="center" class=detail width="20%"><%=m_rs("T46_NO")%></td>
    <td align="center" class=detail width="80%"><%=m_rs("T46_KENMEI")%></td>
    </tr>
<%
    m_rs.MoveNext
    Loop
