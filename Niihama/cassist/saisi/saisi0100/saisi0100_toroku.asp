...
'Response.Write w_sSql & "<br>"
	...
	'* T120の成績データの更新 *
	'**************************
	if wf_UpdateT120(p_iSeiseki,p_iGakusei,p_iNendo,w_sAryHyokaMei,w_iAryHyotei,w_bAryUpdateFlg,p_iHyokaCd) = false then
		m_bErrFlg = True
		Exit Function
	end if
...
'********************************************************************************
	...
	if Cint(w_iHyokaCd) <> Cint(mC_HYOKA_CD_NO) then
		...
		w_sSql = w_sSql & "		T17_HYOTEI_KIMATU_K = " & w_sHyotei & ", "
...
'********************************************************************************
	...
	for i = 1 to 5
		...
		if p_bAryUpdateFlg(i) = true then
			if i = 1 then
				w_sSql = w_sSql & " 	T120_SAISI_SEISEKI = " & p_iSeiseki(i)   & ","
				w_sSql = w_sSql & " 	T120_HYOKA = '"  & p_sAryHyokaMei(i)  & "',"
				w_sSql = w_sSql & " 	T120_HYOTEI = "  & p_iAryHyotei(i) & ","
			else
				w_sSql = w_sSql & " 	T120_SAISI_SEISEKI" & i & " = "  & p_iSeiseki(i)   & ","
				w_sSql = w_sSql & " 	T120_HYOKA"   & i & " = '" & p_sAryHyokaMei(i)  & "',"
				w_sSql = w_sSql & " 	T120_HYOTEI"  & i & " = "  & p_iAryHyotei(i) & ","
			end if
		end if
