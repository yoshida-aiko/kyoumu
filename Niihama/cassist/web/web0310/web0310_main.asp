...
' システム名: 教務事務システム
' 処  理  名: 時間割交換連絡
' ﾌﾟﾛｸﾞﾗﾑID : web/web0310/web0310_main.asp
' 機      能: 上ページ 表示情報を表示
'-------------------------------------------------------------------------
...
Sub s_SetParam()
    ...
    If Request("txtPageCD") <> "" Then
        m_sPageCD = CINT(Request("txtPageCD"))   ':表示済表示頁数（自分自身から受け取る引数）
    Else
...
<table width="100%" border="1" CLASS="hyo">
    <TR>
        <TH CLASS="header" width="55%" nowrap>内　容</TH>
...
%>
    <center>
        <span class="msg">時間割交換連絡データは存在しません。<br>｢新規登録はこちら｣をクリックしてください。</span>
    </center>
<%
    '---------- HTML END   ----------
End Sub
