	...
	<!--
	    ...
	    function f_Touroku(){
			...
			for(i=1; i < <%= m_rCnt %>; i++){
				...
				var obj = eval("document.frm.Seiseki"+i);
...
<% IF w_IdouCnt = 1 Then %>
							<%If m_lKikan = 1 Then%>
								...
								<input type="hidden" class='num' align="center"  name="Seiseki<%=i%>" value="<%=gf_SetNull2Zero(m_SeitoRs("T33_TOKUTEN"))%>" size=2 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>)">
								<%=gf_SetNull2Zero(m_SeitoRs("T33_TOKUTEN"))%>
								</td>
							<% Else %>
								<td class="<%=w_cell%>" width="30"><input type="text" class='num' align="center"  name="Seiseki<%=i%>" value="<%=gf_SetNull2Zero(m_SeitoRs("T33_TOKUTEN"))%>" size=2 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>)"></td>
							<% End IF %>
