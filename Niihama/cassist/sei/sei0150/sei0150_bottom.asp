...
' 変      更: 2016/04/12 西村　 「追加科目開設時時期設定」処理にて開設時時期を変更できるため、T15とT16の開設時期は異なる。よって開設時期はT16を参照するようにする
' 変      更: 2018/10/11 清本 混合学級対応
'*************************************************************************/
%>
...
'Response.Write "[6]"
    ...
    '//前期のみ開設か通年か調べる
'//Del_s 2016/04/12 Nishimura
'    if not f_SikenInfo(m_bZenkiOnly) then
'      m_bErrFlg = True
'      Exit Do
'    end if
'//Del_e 2016/04/12 Nishimura

'//Ins_s 2016/04/12 Nishimura
'// 開設時期はT16を参照するようにする
    if not f_SikenInfo_T16(m_bZenkiOnly) then
      m_bErrFlg = True
      Exit Do
    end if
'//Ins_e 2016/04/12 Nishimura
...
Function f_SikenInfo_T16(p_bZenkiOnly)
  ...
  w_sSQL = w_sSQL & " 	,T13_GAKU_NEN "	'2018.10.11 Add Kiyomoto 混合学級対応
  w_sSQL = w_sSQL & " WHERE "
  w_sSQL = w_sSQL & "   T16_NENDO = " & Cint(m_iNendo)
'  w_sSQL = w_sSQL & " AND T16_GAKKA_CD = '" & m_sGakkaCd & "'"			'2018.10.11 Del Kiyomoto 混合学級対応
  w_sSQL = w_sSQL & " AND T16_KAMOKU_CD= '" & Trim(m_sKamokuCd) & "'"
  'w_sSQL = w_sSQL & " AND T16_KAISETU = " & C_KAI_ZENKI

  '2018.10.11 Add Kiyomoto 混合学級対応 -->
...
<!--
  ...
  function f_Cancel(){
    ...
    function f_Print(){
		...
		arr[7] = "'<%=m_sSyubetu%>'";
		arr[8] = <%=m_iKamokuKbn%>;
		arr[9] = <%=m_iSeisekiInpType%>;
		url = "sei0150_print.asp?txtNendo=";
		url = url + <%=m_iNendo%>;
		url = url + "&txtKyokanCd=";
		url = url + <%=m_sKyokanCd%>;
		url = url + "&txtGakuNo=";
...
//		url = url + <%=m_sKamokuCd%>;
		...
		url = url + "&SYUBETU=";
		url = url + "'<%=m_sSyubetu%>'";
		url = url + "&hidKamokuKbn=";
		url = url + <%=m_iKamokuKbn%>;
		w   = 800;
		h   = 600;

		url = "sei0150_print.asp?arr=" + arr;
		wn  = "SubWindow";
		opt = "directoris=0,location=0,menubar=0,scrollbars=1,status=0,toolbar=0,resizable=yes";
//		if (w > 0)
//			opt = opt + ",width=" + w;
//		if (h > 0)
//			opt = opt + ",height=" + h;
      ...
      <tr>
        <td align="center" align="center" colspan="13">
          ...
          <%End If%>
			...
			<%if m_sGakkoNO = cstr(C_NCT_NIIHAMA) THEN %>
			<input type="button" class="button" value="　印　刷　" onclick="f_Print();">
			<% end if %>
        </td>
      </tr>
    </table>
