...
' SQL実行時のエラー処理
	...
	if Request.Form("PageCount")="" then
		Response.Write "1"
		w_rRs.AbsolutePage = 1
		Session.Contents("PageCount") = w_rRs.AbsolutePage
	else
		iPage = Cint(Request.Form("PageCount"))
		iPage = Cint(Session.Contents("PageCount")) + iPage
		iPage = Cint(iPage)
		w_rRs.AbsolutePage = iPage
	end if
...
<head>
	<title>社員管理</title>
	<base target="INitiran">
</head>
<!-- <BODY BGCOLOR=#F5F5F5> -->
<body>


<table border=1 width=80% align=center bordercolor=#C0C0C0>
...
<% Response.Write Request.Form("w_PageCount") %>
</body>
</html>