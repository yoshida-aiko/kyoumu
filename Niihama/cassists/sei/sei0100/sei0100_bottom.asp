...
Function f_getdate(p_iKamoku_Kbn)
	...
	Do
		...
		if m_bNiteiFlg then
			w_sSQL = w_sSQL & " A.T16_HTEN_TYUKAN_Z AS SEI1,"
			w_sSQL = w_sSQL & " A.T16_HTEN_KIMATU_Z AS SEI2,"
			w_sSQL = w_sSQL & " A.T16_HTEN_TYUKAN_K AS SEI3,"
			w_sSQL = w_sSQL & " A.T16_HTEN_KIMATU_K AS SEI4, "
		else
			w_sSQL = w_sSQL & " A.T16_SEI_TYUKAN_Z AS SEI1,"
			w_sSQL = w_sSQL & " A.T16_SEI_KIMATU_Z AS SEI2,"
			w_sSQL = w_sSQL & " A.T16_SEI_TYUKAN_K AS SEI3,"
			w_sSQL = w_sSQL & " A.T16_SEI_KIMATU_K AS SEI4, "
		end if
		...
		Select Case m_sSikenKBN
			Case C_SIKEN_ZEN_TYU
				if m_bNiteiFlg then
					w_sSQL = w_sSQL & " 	A.T16_HTEN_TYUKAN_Z AS SEI,"
				else
					w_sSQL = w_sSQL & " 	A.T16_SEI_TYUKAN_Z AS SEI,"
				end if
				
				w_sSQL = w_sSQL & " 	A.T16_KEKA_TYUKAN_Z AS KEKA,A.T16_KEKA_NASI_TYUKAN_Z AS KEKA_NASI,A.T16_CHIKAI_TYUKAN_Z AS CHIKAI,A.T16_HYOKAYOTEI_TYUKAN_Z AS HYOKAYOTEI, "
				w_sSQL = w_sSQL & "		A.T16_SOJIKAN_TYUKAN_Z as SOUJI, A.T16_JUNJIKAN_TYUKAN_Z as JYUNJI, "
			Case C_SIKEN_ZEN_KIM
				if m_bNiteiFlg then
					w_sSQL = w_sSQL & " 	A.T16_HTEN_KIMATU_Z AS SEI,"
				else
					w_sSQL = w_sSQL & " 	A.T16_SEI_KIMATU_Z AS SEI,"
				end if
				
				w_sSQL = w_sSQL & " 	A.T16_KEKA_KIMATU_Z AS KEKA,A.T16_KEKA_NASI_KIMATU_Z AS KEKA_NASI,A.T16_CHIKAI_KIMATU_Z AS CHIKAI,A.T16_HYOKAYOTEI_KIMATU_Z AS HYOKAYOTEI, "
				w_sSQL = w_sSQL & "		A.T16_SOJIKAN_KIMATU_Z as SOUJI, A.T16_JUNJIKAN_KIMATU_Z as JYUNJI, "
			Case C_SIKEN_KOU_TYU
				if m_bNiteiFlg then
					w_sSQL = w_sSQL & " 	A.T16_HTEN_TYUKAN_K AS SEI,"
				else
					w_sSQL = w_sSQL & " 	A.T16_SEI_TYUKAN_K AS SEI,"
				end if
				
				w_sSQL = w_sSQL & " 	A.T16_KEKA_TYUKAN_K AS KEKA,A.T16_KEKA_NASI_TYUKAN_K AS KEKA_NASI,A.T16_CHIKAI_TYUKAN_K AS CHIKAI,A.T16_HYOKAYOTEI_TYUKAN_K AS HYOKAYOTEI, "
				w_sSQL = w_sSQL & "		A.T16_SOJIKAN_TYUKAN_K as SOUJI, A.T16_JUNJIKAN_TYUKAN_K as JYUNJI, "
				
			Case C_SIKEN_KOU_KIM
				if m_bNiteiFlg then
					w_sSQL = w_sSQL & " 	A.T16_HTEN_TYUKAN_Z AS SEI_ZT,"
					w_sSQL = w_sSQL & " 	A.T16_HTEN_KIMATU_Z AS SEI_ZK,"
					w_sSQL = w_sSQL & " 	A.T16_HTEN_TYUKAN_K AS SEI_KT,"
					w_sSQL = w_sSQL & " 	A.T16_HTEN_KIMATU_K AS SEI_KK,"
					w_sSQL = w_sSQL & " 	A.T16_HTEN_KIMATU_K AS SEI,"
				else
					w_sSQL = w_sSQL & " 	A.T16_SEI_TYUKAN_Z AS SEI_ZT,"
					w_sSQL = w_sSQL & " 	A.T16_SEI_KIMATU_Z AS SEI_ZK,"
					w_sSQL = w_sSQL & " 	A.T16_SEI_TYUKAN_K AS SEI_KT,"
					w_sSQL = w_sSQL & " 	A.T16_SEI_KIMATU_K AS SEI_KK,"
					w_sSQL = w_sSQL & " 	A.T16_SEI_KIMATU_K AS SEI,"
				end if
				
				w_sSQL = w_sSQL & " 	A.T16_KEKA_TYUKAN_Z AS KEKA_ZT,A.T16_KEKA_NASI_TYUKAN_Z AS KEKA_NASI_ZT,A.T16_CHIKAI_TYUKAN_Z AS CHIKAI_ZT,A.T16_HYOKAYOTEI_TYUKAN_Z AS HYOKAYOTEI_ZT, "
				w_sSQL = w_sSQL & " 	A.T16_KEKA_KIMATU_Z AS KEKA_ZK,A.T16_KEKA_NASI_KIMATU_Z AS KEKA_NASI_ZK,A.T16_CHIKAI_KIMATU_Z AS CHIKAI_ZK,A.T16_HYOKAYOTEI_KIMATU_Z AS HYOKAYOTEI_ZK, "
				w_sSQL = w_sSQL & " 	A.T16_KEKA_TYUKAN_K AS KEKA_KT,A.T16_KEKA_NASI_TYUKAN_K AS KEKA_NASI_KT,A.T16_CHIKAI_TYUKAN_K AS CHIKAI_KT,A.T16_HYOKAYOTEI_TYUKAN_K AS HYOKAYOTEI_KT, "
				
				w_sSQL = w_sSQL & " 	A.T16_KEKA_KIMATU_K AS KEKA,A.T16_KEKA_NASI_KIMATU_K AS KEKA_NASI,A.T16_CHIKAI_KIMATU_K AS CHIKAI,A.T16_HYOKAYOTEI_KIMATU_K AS HYOKAYOTEI, "
				w_sSQL = w_sSQL & " 	A.T16_KEKA_KIMATU_K AS KEKA,A.T16_KEKA_NASI_KIMATU_K AS KEKA_NASI,A.T16_CHIKAI_KIMATU_K AS CHIKAI,A.T16_HYOKAYOTEI_KIMATU_K AS HYOKAYOTEI, "
...
<%
	...
	IF CStr(w_SSSS) <> "" Then
		IF Cstr(w_SSSS) <> CStr(C_IDO_FUKUGAKU) AND Cstr(w_SSSS) <> Cstr(C_IDO_TEI_KAIJO) AND Cstr(w_SSSS) <> Cstr(C_IDO_TENKO) _
					AND Cstr(w_SSSS) <> Cstr(C_IDO_TENKA) AND Cstr(w_SSSS) <> Cstr(C_IDO_KOKUHI) Then
			w_SSSS = "[" & w_SSSR & "]"
			w_bNoChange = True
		Else
			w_SSSR = ""
