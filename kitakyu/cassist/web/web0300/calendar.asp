...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Public m_sKikanKaisi		'//教室使用期間開始
	Public m_sKikanEnd			'//教室使用期間終了
	Public m_SDate
	Public m_EDate
	Public m_sDay    	'//日
	Public m_sKyokanNm
...
Function f_GetKyousituName()
    ...
    Do
		'//教室名取得
		w_sSql = w_sSql & vbCrLf & " SELECT "
		w_sSql = w_sSql & vbCrLf & "  M06_KYOSITU.M06_KYOSITUMEI,M06_SIYOKIKAN_KAISI,M06_SIYOKIKAN_END"
...
'response.write w_sSQL & "<br>"
		...
		If rs.EOF = False Then
			...
			m_sKikanKaisi = rs("M06_SIYOKIKAN_KAISI")
			m_sKikanEnd = rs("M06_SIYOKIKAN_END")
		End If
		...
		For i = 0 to myTblLine-1
		   ...
		   For j=0 To 7-1
			...
			If myDat <> "　" Then
				...
				'使用期間内チェック
				w_bLink = True
				w_rDiff = DateDiff("d", m_sKikanKaisi, w_sDay)
				If w_rDiff < 0 Then
					'入力不可
					w_sTdClassColor = "FILLKIKAN"
					w_bLink = False
				End if
				w_rDiff = DateDiff("d",  w_sDay,m_sKikanEnd)
			    'response.write("*" & w_rDiff & "*")
				If w_rDiff < 0 Then
					'入力不可
					w_sTdClassColor = "FILLKIKAN"
					w_bLink = False
...
%>
	...
	<tr>
		<td align="right" nowrap>
			<span class="msg" ><font size="2">使用期間:<%=m_sKikanKaisi%>～<%=m_sKikanEnd%></font></span>
		</td>
	</tr>
	<tr>
	<td align="right" nowrap>
		<span class="msg" ><font size="2">※赤表示：全て埋まっています。<br>黄表示：一部埋まっています。<br>グレー表示：使用期間外です。</font></span>
	</td>
	</tr></table>
