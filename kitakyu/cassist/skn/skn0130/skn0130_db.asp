...
Sub s_SetParam()
    ...
    m_sJissiFLG      = clng(Request("txtJissiFLG"))       ':実施ﾌﾗｸﾞ
...
'response.write "m_sJissiFLG_1=" & m_sJissiFLG & "<br>"
...
'********************************************************************************
'*  [機能]  担当教官テーブルより、メイン教官フラグを取得する。
'*  [引数]  
'*  [戻値]  f_GetMein：レコード件
'*  [説明]  
'********************************************************************************
Function f_GetMein(p_sClass)
	Dim w_iRet
	Dim w_sSQL
	Dim rs
	...
	Do
		...
		w_sSql = w_sSql & vbCrLf & "  T27_MAIN_FLG"
		w_sSql = w_sSql & vbCrLf & " FROM T27_TANTO_KYOKAN"
		w_sSql = w_sSql & vbCrLf & " WHERE  T27_NENDO = " & m_iSyoriNen
		w_sSql = w_sSql & vbCrLf & " and T27_GAKUNEN = " & m_sGakunen
		w_sSql = w_sSql & vbCrLf & " and T27_CLASS = " & p_sClass
		w_sSql = w_sSql & vbCrLf & " and T27_KAMOKU_CD = '" & m_sKamoku & "'"
		w_sSql = w_sSql & vbCrLf & " and T27_KYOKAN_CD = '" & m_iKyokanCd & "'"
		...
		If rs.EOF = False Then
			'試験時間割データの該当レコード件数取得
			f_GetMein = gf_SetNull2Zero(rs("T27_MAIN_FLG"))
		End If
