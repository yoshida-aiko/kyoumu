...
'*******************************************************************************
    ...
    w_sSQL = w_sSQL & "M05_CLASSNO = " & p_cls & " "
    w_sSQL = w_sSQL & ""
...
''2003.04.15 UPD
    w_sSQL = ""
    w_sSQL = w_sSQL & "SELECT "
    w_sSQL = w_sSQL & "T13_GAKUNEN,T13_CLASS, T13_ZAISEKI_KBN,T11_SEIBETU,T11_NYUGAKU_KBN,T13_GAKUSEI_NO "
    w_sSQL = w_sSQL & "FROM T13_GAKU_NEN,T11_GAKUSEKI,M23_GAKKA_GRP "
    w_sSQL = w_sSQL & "Where "
...
'    w_sSQL = w_sSQL & "T13_ZAISEKI_KBN <= " & C_ZAI_TEIGAKU & " AND "
    w_sSQL = w_sSQL & "M23_GROUP ='"& p_sGrp &"' AND "
    w_sSQL = w_sSQL & "T13_NENDO = M23_nendo AND " 
    w_sSQL = w_sSQL & "T13_GAKKA_CD = M23_GAKKA_CD  AND "
    w_sSQL = w_sSQL & "T13_GAKUNEN = M23_GAKUNEN  AND"
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN <= 2 or  "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_1 is null or T13_IDOU_BI_1 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_2 is null or T13_IDOU_BI_2 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_3 is null or T13_IDOU_BI_3 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_4 is null or T13_IDOU_BI_4 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & " (T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_5 is null or T13_IDOU_BI_5 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_6 is null or T13_IDOU_BI_6 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_7 is null or T13_IDOU_BI_7 > '" & gf_YYYY_MM_DD(date(),"/") & "')) and "
    w_sSQL = w_sSQL & "	(T13_ZAISEKI_KBN > 2 and (T13_IDOU_BI_8 is null or T13_IDOU_BI_8 > '" & gf_YYYY_MM_DD(date(),"/") & "'))) "
...
'    w_sSQL = w_sSQL & "    (select M23_GAKKA_CD from M23_GAKKA_GRP where M23_GROUP ='"& p_sGrp &"' and M23_NENDO = " & m_iNENDO & ") "
    w_sSQL = w_sSQL & "ORDER BY T13_GAKUNEN,T13_CLASS "
'2003.04.15 UPD
'    w_sSQL = w_sSQL & ""
    '== ﾚｺｰﾄﾞｾｯﾄ取得 ==
...
'    response.write w_sSQL
    w_iRet = gf_GetRecordset_OpenStatic(w_rs, w_sSQL)
'response.write gf_GetRsCount(w_rs)&"<BR>"
    If w_iRet <> 0 Then
        '== 取得されなかった場合 ==
        Exit Function
    End If
...
'2003.04.15	if w_iGak >0 and w_iGak <= 6 and w_iCls > 0 and w_iCls <= m_iGrp_su then 
...
'    w_sSQL = w_sSQL & "T11_NYUNENDO = ("&m_iNENDO&" - T13_GAKUNEN + 1) AND "
    ...
    w_sSQL = w_sSQL & "T13_GAKKA_CD IN "
    w_sSQL = w_sSQL & "    (select M23_GAKKA_CD from M23_GAKKA_GRP where M23_GROUP ='"& p_sGrp &"') "
    w_sSQL = w_sSQL & ""
    '== ﾚｺｰﾄﾞｾｯﾄ取得 ==
