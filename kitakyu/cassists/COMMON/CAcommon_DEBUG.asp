...
'           : 2005.11.22 西村        '//gf_Get_SikenKbn()関数の試験区分取得
'*************************************************************************/
...
'response.write w_sSQL & "<br>"
		...
		Else
			...
			'INS_S 2005/11/22 西村 試験が前期中間がない場合もあるので　
	        w_sSQL = ""
	        w_sSQL = w_sSQL & " SELECT "
	        w_sSQL = w_sSQL & "    MIN(M01_SYOBUNRUI_CD) as SIKEN_KBN"
	        w_sSQL = w_sSQL & " FROM M01_KUBUN"
	        w_sSQL = w_sSQL & " WHERE "
	        w_sSQL = w_sSQL & "       M01_NENDO = " & session("NENDO")
	        w_sSQL = w_sSQL & "   AND M01_DAIBUNRUI_CD = " & C_SIKEN

	        iRet = gf_GetRecordset(rs2, w_sSQL)
	        If iRet <> 0 Then
	            'ﾚｺｰﾄﾞｾｯﾄの取得失敗
	            msMsg = Err.description
	            gf_Get_SikenKbn = 99
	        ...
	        If ISNULL(rs2("SIKEN_KBN")) = False Then
	            p_iSiken_Kbn = rs2("SIKEN_KBN")
			Else
 	           p_iSiken_Kbn = C_SIKEN_ZEN_TYU
			END if
		    Call gf_closeObject(rs2)
			'INS_E 2005/11/22 西村
...
' 　　　　：2002/06/13　shin
'*******************************************************************************
Function gf_GetStartEnd(p_Mode,p_SyoriNen,p_Syubetu,p_sSikenKbn,p_sGakunen,p_ClassNo,p_Kamoku,p_sKaisibi,p_sSyuryobi,p_ShikenInsertKbn,p_Flg)
...
p_Flg = "w_iNendo = " & w_iNendo
p_Flg = p_Flg & "p_sGakunen = " & p_sGakunen
p_Flg = p_Flg & "p_sSikenKbn = " & p_sSikenKbn - 1
...
' 備　　考：なし
'*******************************************************************************
Function gf_GetSyukketuData2(p_oRecordset,p_sSikenKbn,p_sGakunen,p_sClass,p_sKamokuCD,p_Nendo,p_ShikenInsertType,p_Syubetu,p_sSql)
...
p_sSql = "p_Nendo = " & p_Nendo & ";p_Syubetu = " & p_Syubetu & ";p_sSikenKbn = " & p_sSikenKbn & ";p_sGakunen = " & p_sGakunen & ";p_sClass = " & p_sClass & ";p_sKamokuCD = " & p_sKamokuCD & ";w_sKaisibi = " & w_sKaisibi & ";w_sSyuryobi = " & w_sSyuryobi
'	p_sSql = "p_Nendo = " & p_Nendo & ";p_Syubetu = " & p_Syubetu & ";p_sSikenKbn = " & p_sSikenKbn & ";p_sGakunen = " & p_sGakunen & ";p_sClass = " & p_sClass & ";p_sKamokuCD = " & p_sKamokuCD

	'== 出欠を取得する開始日と終了日を取得する ==
	'//(試験間の期間)
	if not gf_GetStartEnd("other",p_Nendo,p_Syubetu,p_sSikenKbn,p_sGakunen,p_sClass,p_sKamokuCD,w_sKaisibi,w_sSyuryobi,p_ShikenInsertType,p_sSql) then
		Exit Function
	End If
