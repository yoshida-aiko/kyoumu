...
' システム名: 教務事務システム
' 処  理  名: 成績登録
' ﾌﾟﾛｸﾞﾗﾑID : sei/sei0100/sei0150_middle_23.asp
' 機      能: 下ページ 成績登録の検索を行う
'-------------------------------------------------------------------------
' 引      数:
'           :
...
' 修　    正: 2005/10/28 西村 岐阜高専の場合、総授業時間数は表示しない
'*************************************************************************/
%>
			...
			<td align="center" valign="bottom" nowrap>
				<table class="hyo" border="1" align="center" width="<%=m_TableWidth%>">
					<tr><th class="header3" colspan="14" nowrap align="center">
							<!--総授業数&nbsp; -->
								<%If m_bSeiInpFlg or m_bKekkaNyuryokuFlg Then%>
								<input type="hidden"  <%=w_sInputClass%> maxlength="3" style="width:30px" name="txtSouJyugyou" value="<%= Request("hidSouJyugyou") %>">
								<% Else %>
							...
							<!--純授業数&nbsp;-->
								...
								<% End if%>　
							授業時間数&nbsp;<%If m_bSeiInpFlg or m_bKekkaNyuryokuFlg Then%><input type="text" <%=w_sInputClass%> maxlength="3" style="width:30px" name="txtJunJyugyou" value="<%= Request("hidJunJyugyou") %>"><% Else %><%= Request("hidJunJyugyou") %><% End if%>　
							&nbsp;&nbsp;
