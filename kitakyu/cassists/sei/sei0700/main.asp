...
' 作      成: 2001/07/02 岩田
' 変      更: 2001/07/02
' 変      更: 2007/01/29 高丘 各リンクのアドレスをDB取得からConst取得に変更
'             2017/03/01 藤 美由紀 WEBアクセスログカスタマイズ
'             2017/03/03 藤 美由紀 ○○一覧 表示対象を過去６年分に変更
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
    ...
    Public  m_sTaisyo				'対象		'add 2017/03/01 m.tou
    Public  m_sSosa					'操作		'add 2017/03/01 m.tou
	Public  m_sUserId				'ログインID	'add 2017/03/01 m.tou
	...
	Public  m_sGakkoNO	              '学校番号 INS 2005/11/11 西村
...
'********************************************************************************
    ...
    Do
       ...
       'レコードセットの取得
		...
		'add start 2017/03/01 m.tou
		if m_TxtMode = "LOG" then
			'操作LOG出力
	        If gf_InsertOpeLog(m_iSyoriNen,m_PgMode,m_sMsgTitle,m_sTaisyo,m_sSosa,m_sUserId) <> 0 Then
	            m_bErrFlg = True
	            Exit Do
	        End If
        End If
...
'response.write " Where文 " & w_sWhere
    ...
    p_sSql = p_sSql & " T76_SIKENMEI||'/'||T76_GAKUNEN||'年/'||T76_CLASSMEI  LOG_TAISYO   , "			'add 2017/03/01 m.tou WEBアクセスログカスタマイズ
    p_sSql = p_sSql & " T76_INS_DATE     "
    p_sSql = p_sSql & " FROM T76_PDF "
    p_sSql = p_sSql & " WHERE "
    p_sSql = p_sSql & w_sWhere
	'add start 2017/03/03 m.tou
	'６年分表示
	p_sSql = p_sSql & " T76_NENDO > " & m_iSyoriNen & "-6 AND "
	'add en    2017/03/03 m.tou
	
 
    'p_sSql = p_sSql & " T76_NENDO = " & m_iSyoriNen & " AND "	DEL2005/06/03西村　本科にあわせる
    p_sSql = p_sSql & " T76_TYOHYO_ID = '" & m_PgMode & "' "
    p_sSql = p_sSql & " ORDER BY "

'	IF m_sGakkoNO = C_NCT_FUKUSHIMA THEN	'INS 2005/11/11 西村			'del 2017/03/03 m.tou
	    p_sSql = p_sSql & " T76_NENDO  DESC, "
'	END IF																	'del 2017/03/03 m.tou
...
<link rel=stylesheet href=../../common/style.css type=text/css>
    ...
    //
    //************************************************************
	function f_detail(p_sUrl,p_taisyo){			//add 2017/03/01 m.tou
	//function f_detail(p_sUrl){		//del 2017/03/01 m.tou

		//add start 2017/03/01 m.tou
		//ログINSERT
		f_insertLog(p_taisyo,"Download");
		//add end   2017/03/01 m.tou
	...
	<!-- ADD START 2017/03/01 m.tou WEBアクセスログカスタマイズ -->
	...
	//
	//************************************************************
	function f_insertLog(p_taisyo,p_sosa){

		document.frm.action="main.asp?p_mode=<%=m_PgMode%>";
		document.frm.target="_self";
		document.frm.txtMode.value = "LOG";
		document.frm.txtPageTyu.value ="<%= m_iPageTyu%>";
		document.frm.LOG_TAISYO.value = p_taisyo;
		document.frm.LOG_SOSA.value = p_sosa;
		document.frm.submit();
		
	}
	
	<!-- ADD END   2017/03/01 m.tou WEBアクセスログカスタマイズ -->
	...
	<% Else %>
		...
		<%=w_pageBar %>
			...
			<table border="0" width="100%">
				<tr>
					...
					<% else %>
						...
						<!-- UPD START 2017/03/01 m.tou WEBアクセスログカスタマイズ -->
						<font color="red">ダウンロードする場合は、「Download」ボタンをクリックしてください。<br><br></font>
						...
						<!-- UPD END   2017/03/01 m.tou WEBアクセスログカスタマイズ -->

						<!-- UPD START 2017/03/01 m.tou WEBアクセスログカスタマイズ -->
						<table border="1" width="565" class=hyo>
						...
						<!-- UPD END   2017/03/01 m.tou WEBアクセスログカスタマイズ -->
							<tr>
								...
								<!-- DEL START 2017/03/01 m.tou WEBアクセスログカスタマイズ -->
								<!-- <th align="center" height=16 class=header width="45"nowrap>詳細</th> -->
								<!-- DEL END   2017/03/01 m.tou WEBアクセスログカスタマイズ -->
			...
			'修正 2007/01/29 PDF出力のPathを、CONST.aspに持つようににしました'
								dim w_sPath
								w_sPath = C_PDF_HTTP_PATH
							%>
								<tr>
									...
									<!-- UPD START 2017/03/01 m.tou WEBアクセスログカスタマイズ -->
									<!-- UPD START 2018/03/02 kiyomoto Path修正 -->
									<!--<td align="center" height="16" class=<%=w_cell%> width="100"nowrap><input type=button class=button value="Download" onclick="f_detail('../..<%=gf_HTMLTableSTR(m_Rs("T76_PATH")) %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>','<%=gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');"></td>-->
									<td align="center" height="16" class=<%=w_cell%> width="100"nowrap><input type=button class=button value="Download" onclick="f_detail('../..<%=w_sPath %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>','<%=gf_HTMLTableSTR(m_Rs("LOG_TAISYO")) %>');"></td>
									<!-- <td align="center" height="16" class=<%=w_cell%> width="45"nowrap><input type=button class=button value="詳細" onclick="f_detail('../..<%=w_sPath %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>');"></td> -->
									<!-- <td align="center" height="16" class=<%=w_cell%> width="100"nowrap><a href='../..<%= w_sPath %><%=gf_HTMLTableSTR(m_Rs("T76_FILENAME")) %>'>Download</a></td> -->
									<!-- UPD END   2017/03/01 m.tou WEBアクセスログカスタマイズ -->
								</tr>
							<%
							'以前のソースを保存'2007/01/29
							If false Then 
							%>
								<tr>
									<td align="center" height="16" class=<%=w_cell%> width="200"nowrap><%=gf_HTMLTableSTR(m_Rs("T76_SIKENMEI")) %>&nbsp</td>
							...
							<%
							End If
							'以前のソースを保存'2007/01/29 ここまで
			'修正 2007/01/29 PDF出力のPathを、CONST.aspに持つようににしました'ここまで
								w_iCnt = w_iCnt + 1
								m_Rs.MoveNext
							Loop
							%>
