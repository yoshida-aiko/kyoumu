...
Function f_Update(p_sSikenKBN)
	...
	Do
		w_Today = gf_YYYY_MM_DD(g_iNendo & "/" & month(date()) & "/" & day(date()),"/")
...
'							w_sSQL = w_sSQL & " T16_HYOKA_KIMATU_Z = '', "
							...
							'前期開設科目ならば学年末成績にも登録する INS 2004.10.27 西村
							if m_bZenkiOnly = TRUE THEN
								if not m_bNiteiFlg then
...
%>
	...
	<input type="hidden" name="txtSyoriNendo"     value="<%=trim(Request("txtSyoriNendo"))%>">
