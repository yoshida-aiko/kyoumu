			...
			<form name="frm" method="post">
				<table border=1 class=hyo width=670>
					...
					<tr>
						...
						<th class=header3 width="96"  align="center">欠課入力期間</th><td class=detail width="239"  align="center" colspan="2"><%=m_FromKekka%> ～ <%=m_ToKekka%></td>
					</tr>
					<tr>
				...
				<table class="hyo" border="1" align="center" width="<%=m_TableWidth%>">
					...
					<tr>
						...
						<% Select Case m_sGakkoNO 
							   '北九州は遅刻・欠課の日々計が非表示の為、氏名で幅調整をする ADD 2006/11/21 新谷
							   Case C_NCT_KITAKYU %>
								<th class="header3" rowspan="2" width="<%=gf_IIF(m_KekkaGaiDispFlg,260,315)%>" nowrap>氏　名</th>
						<%    '岐阜は遅刻の非表示・欠課の日々計非表示がある為、氏名で幅調整をする ADD 2006/11/21 新谷
							   Case C_NCT_GIFU %>
								<th class="header3" rowspan="2" width="<%=gf_IIF(m_KekkaGaiDispFlg,315,370)%>" nowrap>氏　名</th>
						<%     Case Else %>
								<th class="header3" rowspan="2" width="<%=gf_IIF(m_KekkaGaiDispFlg,150,260)%>" nowrap>氏　名</th>
						<% End Select %>
						...
						<%if m_sGakkoNO = C_NCT_GIFU then
							'岐阜は遅刻は非表示 INS2005/09/30 西村
						%>
							<th class="header3" colspan="<%=gf_IIF(m_KekkaGaiDispFlg,2,1)%>" nowrap>欠課</th>

						<% ELSEIF  m_sGakkoNO = C_NCT_KITAKYU THEN %>
							<th class="header3" colspan="1" width="55" nowrap>遅刻</th>
						    <th class="header3" colspan="<%=gf_IIF(m_KekkaGaiDispFlg,2,1)%>" width="<%=gf_IIF(m_KekkaGaiDispFlg,110,55)%>" nowrap">欠課</th>
						<% ELSE %>
							<th class="header3" colspan="2" width="110" nowrap>遅刻</th>
						    <th class="header3" colspan="<%=gf_IIF(m_KekkaGaiDispFlg,3,2)%>" width="<%=gf_IIF(m_KekkaGaiDispFlg,165,110)%>" nowrap">欠課</th>
						<% end if %>
					...
					<%if m_sGakkoNO = C_NCT_KITAKYU then
							'北九州は中間非表示 INS2005/09/30   西村
					%>
						<th class="header2" width="60" colspan="2" nowrap><span style="font-size:10px;">前末</span></th>
						<th class="header2" width="60" colspan="2" nowrap><span style="font-size:10px;">学末</span></th>
					<% ELSE %>
					...
					<% END IF %>
						...
						<% ELSE %>
							...
							<% if m_bKekkaNyuryokuFlg then %>
								<th class="header2" nowrap onClick="f_Paste('Chikai')"><span style="font-size:10px;"><FONT COLOR="#99CCFF">入力</FONT></span></th>
							<% else %>
								<th class="header2" width="55" nowrap><span style="font-size:10px;">入力</span></th>
							<% end if %>
							
						<%if m_sGakkoNO = C_NCT_KITAKYU then
							'北九州 日々計非表示 2006.11.20高丘
						%>
						<% ELSE %>
						...
						<%if m_sGakkoNO = C_NCT_GIFU OR m_sGakkoNO = C_NCT_KITAKYU then
							'岐阜は欠課日々計は非表示 INS2005/09/30 西村　
							'北九も 2006.11.20高丘
						%>
						<% ELSE %>
						<th class="header2" width="55" nowrap>
							<span style="font-size:10px;">日々計</span>
						</th>
						<% END IF %>
					</tr>
				</table>
