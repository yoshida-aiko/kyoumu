...
'-------------------------------------------------------------------------
' 作      成: 2002/06/21 shin
' 変      更: 2002/11/21 matsuo
'			  欠課遅刻の表示制御追加、日々計削除、中間試験削除
'*************************************************************************/
%>
...
'/////////////////////////// ﾓｼﾞｭｰﾙ変数 /////////////////////////////
	...
	Dim m_KekkaDispFlg				'//欠課表示フラグ 2002.11.20
...
Sub Main()
	...
	Do
		...
		'//欠課を表示するかチェック 2002.11.20
		if not gf_ChkDisp(C_KEKKA_DISP,m_KekkaDispFlg) then
			m_bErrFlg = True
			Exit Do
		End If
...
Function f_GetStudent()
	...
	'学年クラスに関係なく、その先生の科目を取っている学生を表示 2002.11.20
	w_sSQL = w_sSQL & " WHERE "
	...
	'T16の年度は関係なく、前年までの修得年度が入ってない科目対象
	w_sSQL = w_sSQL & " AND (T16_SYUTOKNENDO = " & Cint(m_iNendo) & " OR T16_SYUTOKNENDO IS NULL)"
	...
	'T27とT16からT15の元々の学科を取得
	w_sSQL = w_sSQL & " AND T16_CURRI_GAKKA_CD IN"
	'T27に学科CDがないので、M05と結合して取得
	w_sSQL = w_sSQL & " ("
	...
	w_sSQL = w_sSQL & " AND M05_NENDO = T27_NENDO"
	w_sSQL = w_sSQL & " )"
...
Sub s_SetTableWidth(p_TableWidth)
	...
	'//欠課表示フラグオンの場合
	if m_KekkaDispFlg then
		p_TableWidth = p_TableWidth + 110
...
<!--
	...
	<%	
		...
		Do Until m_Rs.EOF
			...
			<tr>
				...
				<% if m_iSeisekiInpType <> C_SEISEKI_INP_TYPE_KEKKA then %>
					<td class="<%=w_cell%>" align="center" width="40"  nowrap <%=w_Padding2%>><%=gf_IIF(w_bNoChange,"-",gf_HTMLTableSTR(m_Rs("SEI2")))%></td>
					<td class="<%=w_cell%>" align="center" width="40"  nowrap <%=w_Padding2%>><%=gf_IIF(w_bNoChange,"-",gf_HTMLTableSTR(m_Rs("SEI4")))%></td>
				<% else %>
					<td class="<%=w_cell%>" align="center" width="40"  nowrap <%=w_Padding2%>>-</td>
					<td class="<%=w_cell%>" align="center" width="40"  nowrap <%=w_Padding2%>>-</td>
				<% end if %>
				...
				<% Else %>
					...
					<% if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then %>
						<td class="<%=w_cell%>" align="center" width="50" nowrap <%=w_Padding%>><input type="text" <%=w_sInputClass1%>  name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size=2 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>);" onChange="f_GetTotalAvg();"></td>
			...
			<% if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then %>
				<tr>
					<td class="header" align="right" colspan="5" nowrap>
						<FONT COLOR="#FFFFFF"><B>成績合計</B></FONT>
						<input type="text" name="txtTotal" size="5" <%=w_sInputClass%> readonly>
					</td>
					<% if m_KekkaDispFlg then %>
						<td class="header" align="center" colspan="4" nowrap>&nbsp;</td>
					<% end if %>
				</tr>
	...
	<input type="hidden" name="hidKekkaDispFlg" value ="<%=m_KekkaDispFlg%>">
