...
'出欠データの取得					gf_GetSyukketuData2(p_oRecordset,p_sSikenKbn,p_sGakunen,p_sTantoKyokan,p_sClass,p_sKamokuCD, _
'Ver2.0											p_sKaisibi,p_sSyuryobi,p_s1NenBango,p_Nendo,p_ShikenInsertType,p_GakusekiNo,p_Syubetu)
...
function gf_GetKamokuMei(p_SyoriNen,p_KamokuCd,p_KamokuKbn)
	...
	'通常授業
	if cint(p_KamokuKbn) = cint(C_JIK_JUGYO) then
		w_sSQL = ""
		w_sSQL = w_sSQL & " select "
		...
		w_sSQL = w_sSQL & " 	M03_KAMOKU "
		w_sSQL = w_sSQL & " where "
		w_sSQL = w_sSQL & " 	M03_NENDO =" & cint(p_SyoriNen) & " and "
		w_sSQL = w_sSQL & " 	M03_KAMOKU_CD = " & p_KamokuCd
	'特別活動
	else
		w_sSQL = ""
		w_sSQL = w_sSQL & " select "
		...
		w_sSQL = w_sSQL & " 	M41_TOKUKATU "
		w_sSQL = w_sSQL & " where "
		w_sSQL = w_sSQL & " 	M41_NENDO =" & cint(p_SyoriNen) & " and "
		w_sSQL = w_sSQL & " 	M41_TOKUKATU_CD = " & p_KamokuCd
	end if
