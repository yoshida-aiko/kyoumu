...
'-------------------------------------------------------------------------
' 作      成: 2002/06/20 shin
' 変      更: 2018/06/12 清本 個人履修科目の開設時期を表示条件に追加
...
Function f_GetSubject()
	...
	'--2019/06/17 DELETE FUJIBAYASHI(T16より通常、留学生代替科目を取得するように変更)
	''通常、留学生代替科目取得
	'w_sSQL = ""
	...
	''	w_sSQL = w_sSQL & "    AND M05_GAKKA_CD(+) = T16_GAKKA_CD "	'2018.10.16 Del Kiyomoto 混合学級対応
	...
	w_sSQL = w_sSQL & " 	, M05_CLASS "
	w_sSQL = w_sSQL & " 	, (SELECT T16_NENDO "
	w_sSQL = w_sSQL & " 			, T16_HAITOGAKUNEN "
	w_sSQL = w_sSQL & " 			, T13_CLASS "
	w_sSQL = w_sSQL & " 			, T16_KAMOKU_CD "
	w_sSQL = w_sSQL & " 			, T16_KAISETU "
	w_sSQL = w_sSQL & " 			, MAX(T16_KAMOKU_KBN) AS T16_KAMOKU_KBN "
	w_sSQL = w_sSQL & " 			, MAX(T16_KAMOKUMEI) AS T16_KAMOKUMEI "
	w_sSQL = w_sSQL & " 		FROM T16_RISYU_KOJIN "
	w_sSQL = w_sSQL & " 			,T13_GAKU_NEN "
	w_sSQL = w_sSQL & " 		WHERE T16_NENDO = T13_NENDO "
	w_sSQL = w_sSQL & " 		  AND T16_GAKUSEI_NO = T13_GAKUSEI_NO "
	w_sSQL = w_sSQL & " 		GROUP BY T16_NENDO "
	w_sSQL = w_sSQL & " 			, T16_HAITOGAKUNEN "
	w_sSQL = w_sSQL & " 			, T13_CLASS "
	w_sSQL = w_sSQL & " 			, T16_KAMOKU_CD "
	w_sSQL = w_sSQL & " 			, T16_KAISETU "
	...
	w_sSQL = w_sSQL & " WHERE T27_NENDO = M05_NENDO "
	w_sSQL = w_sSQL & "   AND T27_GAKUNEN = M05_GAKUNEN "
	w_sSQL = w_sSQL & "   AND T27_CLASS = M05_CLASSNO "
	w_sSQL = w_sSQL & "   AND T27_NENDO = T16_NENDO "
	w_sSQL = w_sSQL & "   AND T27_GAKUNEN = T16_HAITOGAKUNEN "
	w_sSQL = w_sSQL & "   AND T27_CLASS = T13_CLASS "
	w_sSQL = w_sSQL & "   AND T27_KAMOKU_CD = T16_KAMOKU_CD "
	w_sSQL = w_sSQL & "   AND T27_NENDO =" & cint(m_iNendo)
	w_sSQL = w_sSQL & "   AND T27_KYOKAN_CD ='" & m_sKyokanCd & "'"
	w_sSQL = w_sSQL & "   AND T27_KAMOKU_BUNRUI = " & C_JIK_JUGYO
	w_sSQL = w_sSQL & "   AND T27_SEISEKI_INP_FLG = " & C_SEISEKI_INP_FLG_YES
	'--2019/06/17 UPDATE END

'2018.06.12 Add Kiyomoto -->
	'//後期期末試験じゃないとき
	if m_iSikenKbn <> C_SIKEN_KOU_KIM then
		w_sSQL = w_sSQL & " AND	("
...
'2018.06.12 Add Kiyomoto <--
	...
	w_sSQL = w_sSQL & " WHERE T26_NENDO = M03_NENDO "
	w_sSQL = w_sSQL & "   AND T26_KAMOKU = M03_KAMOKU_CD "
	w_sSQL = w_sSQL & "   AND T26_NENDO = M05_NENDO "
	w_sSQL = w_sSQL & "   AND T26_GAKUNEN = M05_GAKUNEN "
	w_sSQL = w_sSQL & "   AND T26_CLASS = M05_CLASSNO "
	w_sSQL = w_sSQL & "   AND T26_NENDO = T16_NENDO "
	w_sSQL = w_sSQL & "   AND T26_GAKUNEN = T16_HAITOGAKUNEN "
	w_sSQL = w_sSQL & "   AND T26_CLASS = T13_CLASS "
	w_sSQL = w_sSQL & "   AND T26_KAMOKU = T16_KAMOKU_CD "
	w_sSQL = w_sSQL & "   AND T26_NENDO =" & cint(m_iNendo)
	w_sSQL = w_sSQL & "   AND ("
	w_sSQL = w_sSQL & "			T26_JISSI_KYOKAN    ='" & m_sKyokanCd & "'"
	w_sSQL = w_sSQL & "			OR T26_SEISEKI_KYOKAN1 ='" & m_sKyokanCd & "'"
	w_sSQL = w_sSQL & "			OR T26_SEISEKI_KYOKAN2 ='" & m_sKyokanCd & "'"
											...
											If cint(m_iKongou) = cint(C_CLASS_KONGO) Then
												If cint(gRs("KAMOKU_KBN_IS")) = cint(C_KAMOKU_SENMON) Then
													...
													If m_sGakkoNO <> cstr(C_NCT_OKINAWA) Then
														...
														If m_sGakkoNO = cstr(C_NCT_MAIZURU) AND cint(gRs("GAKUNEN")) = 1 Then
															'INS 2007/06/11
