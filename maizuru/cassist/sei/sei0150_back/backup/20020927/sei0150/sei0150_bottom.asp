...
<!--
	...
	function f_SetSeiseki(w_num){
		...
		ob[0] = eval("top.topFrame.document.frm.sltHyoka");
		...
		Do Until m_Rs.EOF
			...
			<tr>
				...
				<% Else %>
					...
					<% if m_iSeisekiInpType = C_SEISEKI_INP_TYPE_NUM then %>
						<td class="<%=w_cell%>" align="center" width="50" nowrap <%=w_Padding%>><input type="text" <%=w_sInputClass1%>  name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size=2 maxlength=3 onKeyDown="f_MoveCur('Seiseki',this.form,<%=i%>);" onChange="f_GetTotalAvg();" <%=w_Disabled2%>></td>
						
					<% elseif m_iSeisekiInpType = C_SEISEKI_INP_TYPE_STRING then %>
						<td class="<%=w_cell%>" align="center" width="50" nowrap <%=w_Padding%>><input type="button" class="<%=w_cell%>" style="text-align:center;" name="Seiseki<%=i%>" value="<%=w_sSeiseki%>" size=2 onClick="f_SetSeiseki(<%=i%>);" <%=w_Disabled2%>></td>
