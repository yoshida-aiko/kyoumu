...
'Response.end				
				...
				'2018.10.10 Upd Kiyomoto 混合学級対応 -->
				'w_sWhere = w_sWhere & "(T76_GAKUNEN = " & m_RsGakka("M23_GAKUNEN") & " And T76_CLASS = " & w_iClassNo & ") "				

				if Not f_GetClassMulti(m_iSyoriNen,m_RsGakka("M23_GAKUNEN"),w_iClassMulti,m_RsGakka("M23_GAKKA_CD")) then
					m_bErrFlg = True
					m_sErrMsg = "クラスデータの取得に失敗しました。"
					Exit sub				
				end if

				w_sWhere = w_sWhere & "(T76_GAKUNEN = " & m_RsGakka("M23_GAKUNEN") & " And T76_CLASS IN ("

				For Each w_iClass in w_iClassMulti '全要素に対して繰り返す
				    ',で文字列をつなげる
				    w_sWhere = w_sWhere & w_iClass & ","
				Next
				w_sWhere = Mid( w_sWhere, 1, Len(w_sWhere) - 1 )	'最後のカンマ削除
				w_sWhere = w_sWhere & ")) "				
				'2018.10.10 Upd Kiyomoto <--
...
'2018.10.10 Upd Kiomoto -->
'********************************************************************************
'*  [機能]  学科ＣＤからクラスコードを取得する	混合学級対応
'*  [引数]  p_iNendo   ：処理年度
'*          p_sKyokanCd：教官コード
'*          p_iGakunen ：学年
'*          p_iClassNo ：クラスNO
...
'*  [説明]  
'********************************************************************************
Function f_GetClassMulti(p_iNendo,p_iGakunen,w_iClassMulti,p_iGakkaCD)
	Dim w_iRet
	Dim w_sSQL
	Dim rs
	Dim w_iCnt
	...
	w_sSql = w_sSql & vbCrLf & "  T13_CLASS "
	w_sSql = w_sSql & vbCrLf & " FROM T13_GAKU_NEN"
	w_sSql = w_sSql & vbCrLf & " WHERE "
	w_sSql = w_sSql & vbCrLf & "      T13_NENDO=" & p_iNendo
	w_sSql = w_sSql & vbCrLf & "  AND T13_GAKUNEN=" & p_iGakunen
	w_sSql = w_sSql & vbCrLf & "  AND T13_GAKKA_CD=" & p_iGakkaCD
	w_sSql = w_sSql & vbCrLf & "  ORDER BY T13_CLASS"
	...
	'//データが取得できたとき
	Do until rs.EOF
		ReDim Preserve w_iClassMulti(w_iCnt)
		w_iClassMulti(w_iCnt) = rs("T13_CLASS")	'クラスNO
		rs.MoveNext
		w_iCnt = w_iCnt + 1
	Loop

	'//戻り値ｾｯﾄ
	f_GetClassMulti = True
